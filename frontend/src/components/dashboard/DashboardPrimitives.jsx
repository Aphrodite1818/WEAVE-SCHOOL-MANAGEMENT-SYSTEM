...
export function DashboardSectionHeader({
  ...
  return (
    ...
    >
      ...
      {action ? (
        <div className="dashboard-section-action shrink-0">{action}</div>
      ) : null}
    </div>
  );
}
    ...
    >
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          <div
            className={cn(
              "mb-4 flex min-w-0 items-center gap-3 sm:mb-5 sm:inline-flex sm:max-w-full",
              ...
              {!hasSchoolLogo ? (
                <p
                  className={cn(
                    ...
                    isBlueHero ? "text-white/75" : "text-text-faint",
                  )}
                >
                  Powered by
                </p>
          ...
          {eyebrow ? (
            <p
              className={cn(
                "text-[11px] font-bold uppercase tracking-[0.16em] text-text-muted sm:text-xs",
                isBlueHero && "text-white/80",
              )}
            >
              {eyebrow}
            </p>
          ...
          <h2
            className={cn(
              "mt-2 text-xl font-semibold leading-tight text-text sm:text-3xl",
              isBlueHero && "text-white",
            )}
          >
            {title}
          </h2>
          {shouldShowDescription && description ? (
            <p
              className={cn(
                "mt-2 max-w-3xl text-sm leading-6 text-text-muted",
                isBlueHero && "text-white/85",
              )}
            >
              {description}
            </p>
          ...
          {visibleChips.length > 0 ? (
            <div className="mt-3 flex flex-wrap gap-2 sm:mt-4">
              {visibleChips.map((chip) => {
                ...
                const chipToneStyle =
                  toneStyles[chipTone] || toneStyles.neutral;
                return (
                  <span
                    key={`${chip.label}-${chip.value || ""}`}
                    className={cn(
                      "dashboard-welcome-chip inline-flex max-w-full items-center gap-1 rounded-full px-3 py-1 text-[11px] font-bold sm:text-xs",
                      isBlueHero
                  ...
                  >
                    ...
                    {chip.value ? (
                      <span className="truncate opacity-80">{chip.value}</span>
                    ) : null}
                  </span>
                );
              })}
...
export function DashboardMetricCard({
  ...
  return (
    <Card
      ...
      to={to}
      className={cn(
        "group flex flex-col justify-between rounded-2xl border border-border/80 bg-surface transition hover:border-primary/35 hover:shadow-premium",
        compact
          ? "min-h-[5.75rem] p-3"
          : "min-h-[6.75rem] p-3.5 sm:min-h-[8.25rem] sm:p-5",
        to ? "cursor-pointer" : "",
        className,
      )}
    >
      ...
      <div className={compact ? "mt-2" : "mt-3 sm:mt-4"}>
        <p
          className={cn(
            "line-clamp-2 font-semibold text-text-muted",
            compact ? "text-[11px] leading-4" : "text-xs leading-4 sm:text-sm",
          )}
        >
          {label}
        </p>
        <p
          className={cn(
            "mt-1 break-words font-semibold tracking-tight text-text",
        ...
        {description ? (
          <p
            className={cn(
              "mt-1 line-clamp-2 text-text-muted",
              compact
                ? "text-[11px] leading-4"
...
export function DashboardFocusCard({
  ...
  return (
    <Card
      className={cn(
        "flex h-full flex-col rounded-2xl border border-border/80 p-3.5 sm:p-6",
        className,
      )}
    >
      ...
      {primaryAction || secondaryAction ? (
        <div className="mt-4 grid gap-2.5 sm:mt-5 sm:grid-cols-2 sm:gap-3">
          ...
          {secondaryAction ? (
            <DashboardActionButton variant="outline" {...secondaryAction} />
          ) : null}
        </div>
      ) : null}
    </Card>
...
export function DashboardListCard({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="dashboard-attention-list mobile-scroll-list mt-4 grid gap-3"
        style={{
          gridTemplateColumns:
      ...
      >
        {items.length > 0 ? (
          items.map((item) => (
            <DashboardListItem key={item.key || item.title} {...item} />
          ))
        ) : (
...
export function DashboardListItem({
  ...
  return (
    <Wrapper
      to={to}
      className={cn(
        "dashboard-attention-item group flex min-h-[6.75rem] flex-col justify-between rounded-xl border border-border/80 bg-surface px-3 py-3 transition shadow-sm sm:min-h-[8rem] sm:rounded-2xl sm:px-4",
        to
          ? "hover:border-primary/30 hover:bg-primary-subtle/25 hover:shadow-sm"
          : "",
      )}
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {to ? (
          <ChevronRight className="h-4 w-4 shrink-0 text-text-faint transition group-hover:translate-x-0.5" />
        ) : null}
      </div>
      <div className="mt-3 min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <p className="min-w-0 break-words text-sm font-semibold leading-5 text-text">
            {title}
          </p>
          {meta ? (
...
export function DashboardQuickActions({
  ...
  return (
    <Card className="rounded-2xl border border-border/80 p-3.5 sm:p-6">
      <DashboardSectionHeader
        title={title}
        description={description}
      ...
      />
      <div
        className="dashboard-quick-action-grid mt-4 grid gap-3"
        style={{
          gridTemplateColumns:
            "repeat(auto-fit, minmax(min(9.25rem, 100%), 1fr))",
        }}
      >
        {actions.map((action) => {
          const ActionIcon = action.icon;
          const toneStyle =
            toneStyles[action.tone || "primary"] || toneStyles.primary;
          ...
          return (
            <Wrapper
              ...
              className="group flex min-h-[6rem] flex-col rounded-xl border border-border/80 bg-surface px-3 py-3 text-left shadow-sm transition hover:border-primary/30 hover:bg-primary-subtle/25 hover:shadow-premium sm:min-h-[7.5rem] sm:rounded-2xl sm:px-4 sm:py-4"
            >
              <div
                className={cn(
              ...
              <p className="mt-3 break-words text-sm font-semibold leading-5 text-text">
                {action.label}
              </p>
