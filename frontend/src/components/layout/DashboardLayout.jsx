    ...
    <div className="flex h-full min-h-0 flex-col">
      <div
        className={cn(
          ...
          collapsed
            ? "flex-col items-center justify-center h-28 gap-3 pt-2"
      ...
      >
        ...
        {!mobile && (
          <Button
            ...
            size="icon"
            className={cn(
              "hidden h-8 w-8 rounded-lg md:inline-flex transition-all bg-surface-muted hover:bg-border/60",
              collapsed
                ? "relative shadow-sm"
        ...
        <div className="shrink-0 border-t border-border p-4">
          <div className="rounded-2xl border border-border bg-surface px-3 py-3 text-sm text-text-soft">
            ...
            <p className="mt-2 text-xs text-text-muted">
              Contact your school administrator for account or school-data
              issues.
            </p>
          </div>
        </div>
    ...
    <header className="sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur-xl">
      <div className="mx-auto flex h-[60px] w-full max-w-[1320px] items-center gap-2 px-3 sm:h-[76px] sm:px-5 lg:px-8">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[10px] font-bold uppercase tracking-[0.12em] text-text-muted sm:text-xs">
            Learnly AI
          </p>
        ...
        <div className="ml-auto flex items-center gap-2">
          <Dropdown
            ...
            trigger={
              <button
                type="button"
                className="relative inline-flex h-10 w-10 items-center justify-center rounded-full border border-border bg-surface text-text-muted shadow-sm transition hover:bg-surface-muted hover:text-text"
                aria-label="Open notifications"
              >
                <Bell className="h-4 w-4" />
                {unreadCount > 0 && (
                  <span className="absolute -right-1 -top-1 rounded-full bg-error px-1.5 py-0.5 text-[10px] font-bold text-white">
                    {unreadCount}
                  </span>
                )}
              </button>
                ...
                notifications.map((item) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-[11px] text-text-faint">
                      {notificationTimestamp(item.created_at)}
                    </p>
                  </div>
                ))
              ) : (
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-border bg-surface text-text-muted shadow-sm transition hover:bg-surface-muted hover:text-text"
    ...
    <div className="min-h-screen bg-background text-text">
      ...
      {mobileNavOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          />
          <aside className="absolute inset-y-0 left-0 flex h-full w-80 max-w-[85vw] flex-col overflow-hidden border-r border-border bg-surface shadow-2xl">
      ...
      <div
        className={cn(
          ...
          sidebarCollapsed ? "md:pl-[5.5rem]" : "md:pl-72",
        )}
      >
        <Topbar
          role={role}
        ...
        <main
          ...
          ref={mainRef}
          className="mx-auto flex w-full max-w-[1320px] flex-col gap-5 px-3 pb-28 pt-4 sm:gap-6 sm:px-5 sm:pb-12 sm:pt-6 lg:px-8"
        >
          {(title || actions) && (
      ...
      <Modal
        ...
        description={
          profileMode === "onboarding"
            ? profileCopy.onboardingDescription
            : profileCopy.editDescription
        }
