    ...
    <header className="sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur-xl">
      <div className="mx-auto flex h-[58px] w-full max-w-[1320px] items-center gap-1.5 px-2 sm:h-[76px] sm:gap-2 sm:px-5 lg:px-8">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1.5 sm:gap-2">
          ...
          <Dropdown align="right" contentClassName="w-72 max-w-[calc(100vw-1rem)]" trigger={<button type="button" className="flex h-9 items-center gap-2 rounded-full border border-border bg-surface px-1.5 py-1 shadow-sm transition hover:bg-surface-muted sm:h-auto sm:px-2 sm:py-1.5"><Avatar src={avatarSrc} name={userName} size="sm" /><span className="hidden max-w-[10rem] flex-col items-start leading-tight sm:flex"><span className="max-w-full truncate text-sm font-semibold text-text">{userName}</span>{showPlanBadge ? <span className="mt-0.5 text-[10px] font-semibold uppercase tracking-wide text-text-muted">{formatPlanName(planCode)}</span> : null}</span><ChevronDown className="h-4 w-4 text-text-muted" /></button>}>
            <div className="p-2"><div className="mb-2 rounded-2xl border border-border/70 bg-surface-muted/35 px-3 py-3"><div className="flex items-center gap-3"><Avatar src={avatarSrc} name={userName} size="sm" /><div className="min-w-0"><p className="truncate text-sm font-semibold text-text">{userName}</p><p className="truncate text-xs text-text-muted">{roleLabels[role] || "Workspace"}</p></div></div>{showPlanBadge ? <p className="mt-3 text-xs text-text-muted">{formatPlanName(planCode)} - {statusMeta.label}</p> : null}</div><Link to="/profile" className="block rounded-xl px-3 py-2 text-sm font-medium text-text-soft hover:bg-surface-muted">Profile settings</Link><Link to="/legal" className="block rounded-xl px-3 py-2 text-sm font-medium text-text-soft hover:bg-surface-muted">Legal</Link><button type="button" onClick={handleLogout} className="mt-1 flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm font-semibold text-error hover:bg-error-soft"><LogOut className="h-4 w-4" /> Logout</button></div>
          </Dropdown>
        </div>
      </div>
    </header>
