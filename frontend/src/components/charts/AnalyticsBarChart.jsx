...
function AnalyticsBarChart({
  ...
  const renderZeroValueRows = (expanded = false) => (
    <div
      className={`chart-interactive-scroll mt-5 rounded-2xl border border-border bg-surface-muted/30 px-4 py-5 ${expanded ? "max-h-none" : "max-h-[18rem]"}`}
    >
      <div className="grid gap-4">
        {items.map((item, index) => (
          <div key={`${item?.[labelKey]}-${index}`} className="space-y-2">
            <div className="flex items-center justify-between gap-3 text-sm">
              <span className="min-w-0 break-words font-medium text-text">{formatChartLabel(item?.[labelKey])}</span>
              <span className="shrink-0 font-semibold text-text-muted">{Number(item?.[valueKey])}</span>
            </div>
  ...
  const renderChartBody = (expanded = false) => {
    ...
    if (!hasVisibleValues) return renderZeroValueRows(expanded);

    const chartHeight = useHorizontalBars
      ? Math.max(expanded ? 420 : 280, items.length * (expanded ? 56 : 48))
      : expanded
        ? 500
        : isMobileChart
          ? 300
          : 280;
    const yAxisWidth = expanded ? 170 : 138;
    const verticalChartMinWidth = !useHorizontalBars
      ? Math.max(expanded ? 780 : isMobileChart ? 340 : 520, items.length * (expanded ? 140 : isMobileChart ? 88 : 118))
      : expanded
        ? 640
        : "100%";
      ...
      <div className="chart-interactive-scroll mt-5 min-h-0 flex-1 rounded-2xl border border-border/50 bg-surface-muted/10 px-1 py-3 sm:px-2">
        <div style={{ minWidth: verticalChartMinWidth, height: chartHeight }}>
          <ResponsiveContainer width="100%" height="100%">
            {useHorizontalBars ? (
              <BarChart data={items} layout="vertical" margin={{ left: 0, right: expanded ? 28 : 16, top: 8, bottom: 8 }}>
                ...
                <YAxis
                  ...
                  tickMargin={10}
                  width={yAxisWidth}
                  tickFormatter={(value) => shortLabel(value, expanded ? 24 : 18)}
                  tick={{ fontSize: expanded ? 14 : 13, fontWeight: 500 }}
                />
                <Tooltip labelFormatter={formatChartLabel} />
                <Bar dataKey={valueKey} radius={[0, 10, 10, 0]} barSize={expanded ? 24 : 20}>
                  {items.map((item, index) => (
              ...
              <BarChart data={items} margin={{ left: 0, right: expanded ? 28 : 12, top: 8, bottom: isMobileChart ? 36 : 24 }}>
                ...
                <Tooltip labelFormatter={formatChartLabel} />
                <Bar dataKey={valueKey} radius={[10, 10, 0, 0]} maxBarSize={expanded ? 64 : isMobileChart ? 48 : 56}>
                  {items.map((item, index) => (
                    <Cell key={`${item?.[labelKey]}-${index}`} fill={CHART_BAR_COLORS[index % CHART_BAR_COLORS.length]} />
                  ))}
                </Bar>
