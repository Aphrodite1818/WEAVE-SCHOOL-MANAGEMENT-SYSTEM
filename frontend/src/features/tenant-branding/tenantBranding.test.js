...
test("tenant response validation prevents cross-tenant cache use", () => {
  const response = { tenant_id: "school-a", token_schema_version: 4, light_tokens: tokens, dark_tokens: tokens };
  assert.ok(validateBrandingResponse(response, "school-a"));
  assert.equal(validateBrandingResponse(response, "school-b"), null);
  assert.equal(validateBrandingResponse({ ...response, token_schema_version: 3 }, "school-a"), null);
});
...
test("appearance switches token sets without retaining previous variables", () => {
  ...
  const response = {
    tenant_id: "school-a", theme_version: 2, token_schema_version: 4, is_enabled: true,
