...
test("partial failure retains only failed arms for retry and continues the batch", async () => {
  ...
  assert.equal(result.created.length, 2);
  assert.deepEqual(
    result.failed.map((f) => f.armLabelId),
    ["B"],
  );
});

test("activation warning explains structural locks and the editable name", async () => {
  const { readFile } = await import("node:fs/promises");
  const source = await readFile(
    new URL("./AcademicLevelsWorkspace.jsx", import.meta.url),
    "utf8",
  );
  ...
  assert.match(
    source,
    /category, progression position, and department specialization timing become locked after activation/,
  );
  assert.match(source, /even if you deactivate the level later/);
  assert.match(source, /Only the level name remains editable/);
