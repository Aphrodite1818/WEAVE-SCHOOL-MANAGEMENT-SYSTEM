...
function ReportCardsWorkspace({ activeTab, onContextChange }) {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {previewCard ? <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2"><PreviewField label="Student" value={previewCard.student_name || previewCard.admission_number} /><PreviewField label="Class" value={[previewCard.class_name, previewCard.class_arm].filter(Boolean).join(" ")} /><PreviewField label="Session" value={previewCard.academic_session_name} /><PreviewField label="Term" value={String(previewCard.academic_term_name || "").replaceAll("_", " ")} /><PreviewField label="Average" value={previewCard.average_score} /><PreviewField label="Position" value={previewCard.position ? `${previewCard.position}/${previewCard.position_out_of || "–"}` : "–"} /></div><div className="overflow-x-auto rounded-2xl border border-border"><table className="min-w-full text-sm"><thead className="bg-surface-muted/40 text-left text-xs uppercase text-text-muted"><tr><th className="px-3 py-3">Subject</th><th className="px-3 py-3">Test</th><th className="px-3 py-3">Assessment</th><th className="px-3 py-3">Exam</th><th className="px-3 py-3">Total</th><th className="px-3 py-3">Grade</th></tr></thead><tbody>{(previewCard.lines || []).map((line) => <tr key={line.id} className="border-t border-border/70"><td className="px-3 py-3 font-medium text-text">{line.subject_name}</td><td className="px-3 py-3">{line.test_score}</td><td className="px-3 py-3">{line.assessment_score}</td><td className="px-3 py-3">{line.exam_score}</td><td className="px-3 py-3 font-semibold">{line.total_score}</td><td className="px-3 py-3">{line.grade}</td></tr>)}</tbody></table></div><div className="grid gap-3 sm:grid-cols-2"><PreviewField label="Class teacher comment" value={previewCard.class_teacher_comment || "No comment"} /><PreviewField label="Principal comment" value={previewCard.principal_comment || "No comment"} /></div></div> : null}
      </Modal>
    </div>
  );
}
