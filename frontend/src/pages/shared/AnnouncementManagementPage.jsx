...
import {
  messageService,
  communicationAnnouncementService,
...
const superadminAudiences = [
  ...
  {
    value: "tenant_admins_of_tenants",
    label: "Tenant admins of selected tenants",
  },
];
...
function buildPayload(form) {
  ...
  if (!selectedActorAudience && form.classId)
    audiences[0].class_id = form.classId;
  if (!selectedActorAudience && form.tenantTargetId)
    audiences[0].tenant_target_id = form.tenantTargetId;
...
const archivedPreferenceKey = (mode) =>
  `weave:${mode}:show-archived-announcements`;
...
function SearchableOptionPicker({
  ...
  const selectedOptions =
    ...
    options.filter((item) => selectedValues.includes(item.id || item.actor_id));
    ...
    <div className="space-y-2">
      ...
      {multi && selectedValues.length ? (
        <p className="text-xs font-semibold text-text-muted">
          {selectedValues.length} audience member
...
export default function AnnouncementManagementPage({ mode = "tenant-admin" }) {
  ...
  const audienceOptions = useMemo(
    () => (mode === "superadmin" ? superadminAudiences : tenantAudiences),
    [mode],
  );
  const recipients = useMemo(
    () => flattenRecipientGroups(recipientGroups),
    [recipientGroups],
  );
  const selectedActorType =
    selectedActorTypeByAudience[form.audienceType] || "";
  ...
  const filteredRecipients = useMemo(() => {
    ...
    return recipients
      ...
      .filter(
        (recipient) => !query || recipient.label.toLowerCase().includes(query),
      )
      .slice(0, 25);
  ...
  const filteredClasses = useMemo(() => {
    ...
    return classes
      .filter(
        (item) => !query || classLabel(item).toLowerCase().includes(query),
      )
      .slice(0, 25);
  }, [classSearch, classes]);
  const filteredTenants = useMemo(() => {
    ...
    return tenants
      .filter(
        (item) =>
          !query ||
          String(item.school_name || item.name || item.email || "")
            .toLowerCase()
            .includes(query),
      )
      .slice(0, 25);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [response, recipientResponse, classResponse, tenantResponse] =
        await Promise.all([
          communicationAnnouncementService.list(mode, {
            skip,
          ...
          mode === "superadmin"
            ? superadminService.getTenants(0, 100, false)
            : Promise.resolve({ items: [] }),
        ]);
  ...
  const previewAudience = async () => {
    setFormError("");
    try {
      const response = await communicationAnnouncementService.preview(
        mode,
        buildPayload(form),
      );
      setPreview(response);
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (!Number(response?.recipient_count || 0)) {
        setFormError(
          "No active recipients match this audience. Choose a different audience before saving.",
        );
        return;
      }
  ...
  return (
    ...
    >
      <section className="grid w-full gap-4 sm:gap-5 xl:grid-cols-[26rem_1fr]">
        <form
          onSubmit={submit}
          className="space-y-4 rounded-2xl border border-border bg-surface p-3 sm:p-4"
        >
          <Input
            label="Title"
            value={form.title}
          ...
          {form.audienceType === "tenant_admins_of_tenants" ? (
            <SearchableOptionPicker
              ...
              getOptionLabel={(tenant) =>
                tenant.school_name || tenant.name || tenant.email || "Tenant"
              }
              getOptionMeta={(tenant) =>
                tenant.email || tenant.slug || "Tenant"
              }
          ...
          {preview ? (
            <div className="rounded-lg border border-border bg-surface-muted/40 p-3 text-sm">
              <p className="font-bold text-text">
                Audience: {preview.audience_label}
              </p>
              <p className="mt-1 text-text-muted">
                Recipients: {preview.recipient_count} active recipient
                {preview.recipient_count === 1 ? "" : "s"}
              </p>
              {preview.excluded_reasons?.length ? (
                <p className="mt-1 text-warning">
                  {preview.excluded_reasons.join(", ")}
                </p>
              ) : null}
            </div>
          ) : null}
          {formError ? (
            <div className="rounded-lg border border-error/30 bg-error-soft px-3 py-2 text-sm font-semibold text-error">
              {formError}
            </div>
          ) : null}
          <div className="flex flex-wrap gap-2">
        ...
        <main className="space-y-4">
          <div className="flex flex-col gap-3 rounded-2xl border border-border bg-surface p-3 sm:flex-row sm:items-center sm:justify-between sm:p-4">
            <div>
              ...
              <p className="mt-1 text-sm text-text-muted">
                Archived announcements are hidden from this view unless you
                choose to show them.
              </p>
            </div>
          ...
          {!loading && !error && !visibleItems.length ? (
            <EmptyState
              ...
              description={
                items.length
                  ? "Turn on Show archived when you need to review older announcements."
                  : undefined
              }
            />
          ) : null}
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 line-clamp-3 max-w-3xl text-xs text-text-muted sm:text-sm">
                        {item.body}
                      </p>
                      <p className="mt-2 text-xs font-semibold uppercase text-text-faint">
                        {item.status}
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {item.status === "published" ? (
                        <Button
                          ...
                          onClick={() =>
                            mutate(() =>
                              communicationAnnouncementService.archive(
                                mode,
                                item.id,
                              ),
                            )
                      ...
                      {["draft", "scheduled"].includes(item.status) ? (
                        <>
                          <Button
                            ...
                            onClick={() =>
                              mutate(() =>
                                communicationAnnouncementService.publish(
                                  mode,
                                  item.id,
                                ),
                              )
                          ...
                          <Button
                            ...
                            onClick={() =>
                              mutate(() =>
                                communicationAnnouncementService.cancel(
                                  mode,
                                  item.id,
                                ),
                              )
