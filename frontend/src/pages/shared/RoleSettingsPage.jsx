...
const roleCopy = {
  admin: {
    title: "Settings",
    description:
      "Manage profile, school, guidance, and device accessibility settings.",
  },
  teacher: {
    title: "Settings",
    description:
      "Manage teacher profile, workspace guidance, and device accessibility preferences.",
  },
  student: {
    title: "Settings",
    description:
      "Manage student profile, workspace guidance, and device accessibility preferences.",
  },
  parent: {
    title: "Settings",
    description:
      "Manage parent profile, workspace guidance, and device accessibility preferences.",
  },
};
...
const resolveCurrentEmail = (user) =>
  user?.email || user?.email_address || user?.account_email || "";
const resolveAdmissionNumber = (user) =>
  user?.admission_number ||
  user?.student?.admission_number ||
  user?.profile?.admission_number ||
  "";
...
function SettingsRow({
  ...
  const content = (
    <div
      ...
      className="flex min-h-[3.75rem] items-center gap-3 border-t border-border/70 px-4 py-3 first:border-t-0 sm:px-5"
    >
      ...
      {value ? (
        <p className="max-w-[42%] truncate text-right text-sm text-text-muted">
          {value}
        </p>
      ) : null}
      {actionLabel ? (
  ...
  if (onClick) {
    return (
      <button
        type="button"
        onClick={onClick}
        className="block w-full text-left transition hover:bg-surface-muted/45"
      >
        {content}
      </button>
    );
...
function ExpandableSettingsRow({
  icon: Icon,
  label,
  value,
  open,
  ...
  return (
    <div className="border-t border-border/70 first:border-t-0">
      <button
        type="button"
        onClick={onToggle}
        className="flex min-h-[3.75rem] w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-surface-muted/45 sm:px-5"
        aria-expanded={open}
      >
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-surface-muted text-text-soft">
          <Icon className="h-[18px] w-[18px]" />
        ...
        <div className="min-w-0 flex-1">
          ...
          {value ? (
            <p className="mt-0.5 truncate text-xs text-text-muted">{value}</p>
          ) : null}
        </div>
        {open ? (
      ...
      <div className="mx-auto grid w-full max-w-6xl gap-5 lg:grid-cols-[22rem_minmax(0,1fr)]">
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <Card className="overflow-hidden rounded-[1.5rem] p-0">
            <div className="flex flex-col items-center border-b border-border/70 bg-surface-muted/30 px-5 py-6 text-center">
              ...
              />
              <h2 className="mt-3 max-w-full truncate text-lg font-bold text-text">
                {displayName || "Your account"}
              </h2>
              <p className="mt-1 max-w-full truncate text-sm text-text-muted">
                {profileSummary || normalizedRole}
              </p>
            </div>
            <div className="p-2">
              <Link
                to="/profile"
                className="flex items-center justify-between rounded-2xl px-3 py-3 text-sm font-semibold text-text-soft transition hover:bg-surface-muted hover:text-text"
              >
                Profile details
        ...
        <div className="space-y-5 pb-24 md:pb-0">
          <SettingsGroup title="Account">
            {isStudent ? (
              <SettingsRow
                ...
                value={admissionNumber || "Not assigned"}
              />
            ) : (
              <SettingsRow
                icon={Mail}
                label="Email"
                value={currentEmail || "No email on file"}
              />
            )}
            <SettingsRow
              icon={UserRound}
              label="Profile"
              value={profileSummary || "Details and photo"}
              to="/profile"
            />
          </SettingsGroup>
            ...
            <SettingsGroup title="School">
              ...
              {tenantBrandingGuard.allowed && !tenantBrandingGuard.pending ? (
                <SettingsRow
                  ...
                  description="Manage the shared school identity and colour palette."
                  to="/admin/settings/branding"
                  tourTarget="/admin/settings/branding"
                />
              ) : null}
            </SettingsGroup>
          ) : null}
          ...
          <SettingsGroup title="Appearance and accessibility">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        active
                          ...
                          : "border-border bg-surface text-text-soft hover:bg-surface-muted",
                      )}
                    >
            ...
            {accessibilityStatus ? (
              <p className="border-t border-border/60 px-4 py-3 text-xs font-semibold text-success sm:px-5">
                {accessibilityStatus}
              </p>
            ) : null}
          </SettingsGroup>
