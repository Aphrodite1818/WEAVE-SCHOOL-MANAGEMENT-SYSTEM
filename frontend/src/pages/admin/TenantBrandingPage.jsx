          ...
          <section className="border-b border-border/60 p-5 sm:p-7 xl:border-b-0 xl:border-r">
            ...
            <div className="mt-5 grid gap-2.5 sm:grid-cols-2">
              {PALETTES.map((palette) => {
                ...
                return <button key={palette.key} type="button" onClick={() => setPaletteKey(palette.key)} aria-pressed={active} className={`group flex min-h-20 items-center gap-3 rounded-xl border px-3.5 py-3 text-left transition-all ${active ? "border-primary bg-primary-subtle shadow-sm ring-2 ring-primary/10" : "border-border/70 bg-surface hover:-translate-y-0.5 hover:border-border-strong hover:shadow-sm"}`}><span className="relative h-11 w-11 shrink-0 overflow-hidden rounded-xl shadow-sm"><span className="absolute inset-0" style={{ background: palette.primary }} /><span className="absolute bottom-0 right-0 h-6 w-6 rounded-tl-xl" style={{ background: palette.accent }} /></span><span className="min-w-0 flex-1"><span className="flex items-center justify-between gap-2 font-semibold text-text"><span className="truncate">{palette.name}</span>{active ? <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground"><Check className="h-3 w-3" /></span> : null}</span><span className="mt-0.5 block truncate text-xs text-text-muted">{palette.description}</span></span></button>;
              })}
            </div>
          </section>
