...
function AcademicHubPage() {
  ...
  const activateAssignment = async (assignment) => { setIsSaving(assignment.id); try { const saved = await academicService.activateTeacherAssignment(assignment.id); setAssignments((current) => current.map((item) => (item.id === saved.id ? saved : item))); showSuccess("Assignment reactivated."); } catch (err) { showError(getErrorMessage(err, "Could not reactivate assignment.")); } finally { setIsSaving(""); } };
  ...
  return (
    <DashboardLayout role="admin" title="Academic Hub">
      ...
      <section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6"><Metric label="Sessions" value={stats.sessions} /><Metric label="Terms" value={stats.terms} /><Metric label="Subjects" value={stats.subjects} /><Metric label="Assignments" value={stats.assignments} /><Metric label="Drafts" value={stats.drafts} /><Metric label="Submitted" value={stats.submitted} /></section>
      ...
      <section className="rounded-[1.35rem] border border-border bg-surface p-4 shadow-sm sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-semibold text-text sm:text-xl">{activeTabMeta.label}</h2></div><div className="grid grid-cols-2 gap-2 text-xs sm:min-w-[280px]"><Context label="Current Session" value={currentSession?.name || "Not set"} /><Context label="Current Term" value={currentTerm?.name ? displayTerm(currentTerm.name) : "Not set"} /></div></div></section>
      ...
      {activeTab === "assignments" ? <AssignmentsTab classes={classes} subjects={subjects} teachers={teachers} classSubjects={classSubjects} assignments={assignments} assignmentFilters={assignmentFilters} setAssignmentFilters={setAssignmentFilters} assignmentForm={assignmentForm} setAssignmentForm={setAssignmentForm} visibleAssignments={visibleAssignments} saveAssignment={saveAssignment} editAssignment={editAssignment} resetAssignmentForm={resetAssignmentForm} editingAssignmentId={editingAssignmentId} deactivateAssignment={deactivateAssignment} activateAssignment={activateAssignment} isSaving={isSaving} /> : null}
...
function AssignmentsTab({ classes, subjects, teachers, classSubjects, assignments, assignmentFilters, setAssignmentFilters, assignmentForm, setAssignmentForm, visibleAssignments, saveAssignment, editAssignment, resetAssignmentForm, editingAssignmentId, deactivateAssignment, activateAssignment, isSaving }) {
...
function Panel({ title, children }) { return <section className="rounded-[1.35rem] border border-border bg-surface shadow-sm"><div className="border-b border-border/70 p-4 sm:p-5"><h2 className="text-base font-semibold text-text sm:text-lg">{title}</h2></div><div className="p-4 sm:p-5">{children}</div></section>; }
...
function Metric({ label, value }) { return <div className="h-full rounded-2xl border border-border bg-surface px-3 py-3 shadow-sm sm:px-4"><p className="text-[10px] font-semibold uppercase tracking-wide text-text-muted sm:text-[11px]">{label}</p><p className="mt-1 text-2xl font-semibold leading-none text-text sm:text-[1.9rem]">{value}</p></div>; }
