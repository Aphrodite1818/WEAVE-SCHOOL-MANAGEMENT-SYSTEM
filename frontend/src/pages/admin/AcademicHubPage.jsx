...
function AcademicHubPage() {
  ...
  const termsForResultSession = useMemo(
    () => terms.filter((term) => !resultFilters.academic_session_id || term.academic_session_id === resultFilters.academic_session_id),
    [terms, resultFilters.academic_session_id]
  );
  const termsForReportSession = useMemo(
    () => terms.filter((term) => !reportFilters.academic_session_id || term.academic_session_id === reportFilters.academic_session_id),
    [terms, reportFilters.academic_session_id]
  );

  const visibleAssignments = useMemo(
    () => assignments.filter((item) =>
      (!assignmentFilters.class_id || item.class_id === assignmentFilters.class_id) &&
      (!assignmentFilters.subject_id || item.subject_id === assignmentFilters.subject_id) &&
      (!assignmentFilters.teacher_id || item.teacher_id === assignmentFilters.teacher_id)
    ),
    [assignments, assignmentFilters]
  );

  const selectedAssignment = useMemo(() => {
    if (editingResultId && resultForm.teacher_assignment_id) {
      return assignments.find((item) => item.id === resultForm.teacher_assignment_id);
    }
    return assignments.find((item) =>
      item.is_active && item.class_id === resultFilters.class_id && item.subject_id === resultFilters.subject_id
    );
  }, [assignments, editingResultId, resultForm.teacher_assignment_id, resultFilters.class_id, resultFilters.subject_id]);

  const visibleResults = useMemo(
    () => results.filter((item) => !resultFilters.subject_id || item.subject_id === resultFilters.subject_id),
    [results, resultFilters.subject_id]
  );

  const visibleReportCards = useMemo(
    () => reportCards.filter((card) =>
      (!reportFilters.academic_session_id || card.academic_session_id === reportFilters.academic_session_id) &&
      (!reportFilters.academic_term_id || card.academic_term_id === reportFilters.academic_term_id) &&
      (!reportFilters.class_id || card.class_id === reportFilters.class_id)
    ),
    [reportCards, reportFilters]
  );
    ...
    async function loadFilteredResults() {
      ...
      try {
        const response = await academicService.listAdminResults({
          class_id: resultFilters.class_id,
          academic_session_id: resultFilters.academic_session_id,
          academic_term_id: resultFilters.academic_term_id,
          limit: 100,
        });
        if (mounted) setResults(response?.items || []);
      } catch (err) {
        if (mounted) setResults([]);
        showError(getErrorMessage(err, "Could not load results for this filter."));
      }
    }
    loadFilteredResults();
    return () => { mounted = false; };
  }, [resultFilters.class_id, resultFilters.academic_session_id, resultFilters.academic_term_id]);

  useEffect(() => {
    let mounted = true;
    async function loadOverview() {
      if (activeTab !== "reportCards" || !reportFilters.class_id || !reportFilters.academic_session_id || !reportFilters.academic_term_id) {
    ...
    loadOverview();
    return () => { mounted = false; };
  }, [activeTab, reportFilters.class_id, reportFilters.academic_session_id, reportFilters.academic_term_id]);
  ...
  const updateReportFilter = (key, value) => {
    setReportFilters((current) => ({ ...current, [key]: value, ...(key === "academic_session_id" ? { academic_term_id: "" } : {}) }));
  };
  ...
  const saveAssignment = async (event) => {
    ...
    if (!assignmentForm.class_id || !assignmentForm.class_subject_id || !assignmentForm.teacher_id) {
      showWarning("Select a class, subject, and teacher before saving.");
      return;
    }
    ...
    try {
      ...
      if (assignmentForm.class_id) {
        const classSubjectResponse = await academicService.listClassSubjects(assignmentForm.class_id, { active_only: true });
        setClassSubjects(classSubjectResponse?.items || []);
      }
      setAssignmentFilters({ class_id: saved.class_id || assignmentForm.class_id, subject_id: saved.subject_id || assignmentSubject?.subject_id || "", teacher_id: saved.teacher_id || assignmentForm.teacher_id });
      resetAssignmentForm();
  ...
  const deactivateAssignment = async (assignment) => {
    ...
    } catch (err) { showError(getErrorMessage(err, "Could not deactivate assignment.")); }
    finally { setIsSaving(""); }
  };

  const loadResultsForCurrentFilter = async () => {
    if (!resultFilters.class_id || !resultFilters.academic_session_id || !resultFilters.academic_term_id) return;
  ...
  const saveResult = async (event) => {
    ...
    if (!resultFilters.class_id || !resultFilters.subject_id) {
      showWarning("Select a class and subject before recording scores.");
      return;
    }
    if (!assignmentId) {
      showWarning("Assign a teacher to this class-subject before saving scores.");
      return;
    }
    if (!resultForm.student_id || !resultFilters.academic_session_id || !resultFilters.academic_term_id) {
      showWarning("Select student, session, and term before saving scores.");
      return;
    }
    if (resultForm.status === "submitted" && !hasAllScores(resultForm)) {
      showWarning("All three scores are required before submitting a result.");
      return;
    }
    if (scoreTotal(resultForm) > 100) {
    ...
    try {
      ...
      await loadResultsForCurrentFilter();
  ...
  const regenerateReportCard = async (reportCardId) => {
    ...
    } catch (err) { showError(getErrorMessage(err, "Could not regenerate report card.")); }
    finally { setIsSaving(""); }
  };
  ...
  const editResult = (result) => {
    ...
    setResultFilters({ class_id: result.class_id || "", subject_id: result.subject_id || "", academic_session_id: result.academic_session_id || "", academic_term_id: result.academic_term_id || "" });
    setResultForm({ student_id: result.student_id || "", teacher_assignment_id: result.teacher_assignment_id || "", test_score: result.test_score ?? "", assessment_score: result.assessment_score ?? "", exam_score: result.exam_score ?? "", status: result.status || "draft" });
  };

  if (isLoading) {
    return <DashboardLayout role="admin" title="Academic Hub"><LoadingState label="Loading academic hub..." /></DashboardLayout>;
  }
    ...
    <DashboardLayout role="admin" title="Academic Hub" description="Configure academic setup, teacher assignments, results, and official report cards.">
      ...
      <section className="rounded-[1.35rem] border border-border bg-surface p-3 shadow-sm">
        <div className="flex flex-col gap-2 lg:flex-row">
          {tabs.map((tab) => {
            ...
            return (
              <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`flex flex-1 items-start gap-3 rounded-2xl px-3 py-3 text-left transition ${active ? "bg-primary text-text-inverse shadow-sm" : "text-text-soft hover:bg-surface-muted hover:text-text"}`}>
                <Icon className="mt-0.5 h-4 w-4 shrink-0" />
                <span className="min-w-0"><span className="block text-sm font-semibold">{tab.label}</span><span className={`mt-1 block text-xs leading-5 ${active ? "text-white/80" : "text-text-muted"}`}>{tab.description}</span></span>
              </button>
            );
          })}
        </div>
      ...
      <section className="rounded-[1.35rem] border border-border bg-surface p-4 shadow-sm sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-sm font-semibold text-primary">{activeTabMeta.label}</p><h2 className="mt-1 text-lg font-semibold text-text sm:text-xl">{activeTabMeta.description}</h2></div>
          <div className="grid grid-cols-2 gap-2 text-xs sm:min-w-[280px]"><Context label="Current Session" value={currentSession?.name || "Not set"} /><Context label="Current Term" value={currentTerm?.name ? displayTerm(currentTerm.name) : "Not set"} /></div>
        </div>
      </section>

      {activeTab === "setup" ? (
        <SetupTab sessions={sessions} terms={terms} scales={scales} subjects={subjects} sessionForm={sessionForm} setSessionForm={setSessionForm} termForm={termForm} setTermForm={setTermForm} scaleForm={scaleForm} setScaleForm={setScaleForm} subjectForm={subjectForm} setSubjectForm={setSubjectForm} saveSession={saveSession} saveTerm={saveTerm} saveScale={saveScale} saveSubject={saveSubject} editSession={editSession} editTerm={editTerm} editScale={editScale} editSubject={editSubject} resetSessionForm={resetSessionForm} resetTermForm={resetTermForm} resetScaleForm={resetScaleForm} resetSubjectForm={resetSubjectForm} editingSessionId={editingSessionId} editingTermId={editingTermId} editingScaleId={editingScaleId} editingSubjectId={editingSubjectId} isSaving={isSaving} />
      ) : null}

      {activeTab === "assignments" ? (
        <AssignmentsTab classes={classes} subjects={subjects} teachers={teachers} classSubjects={classSubjects} assignmentFilters={assignmentFilters} setAssignmentFilters={setAssignmentFilters} assignmentForm={assignmentForm} setAssignmentForm={setAssignmentForm} visibleAssignments={visibleAssignments} saveAssignment={saveAssignment} editAssignment={editAssignment} resetAssignmentForm={resetAssignmentForm} editingAssignmentId={editingAssignmentId} deactivateAssignment={deactivateAssignment} isSaving={isSaving} />
      ) : null}

      {activeTab === "results" ? (
        <ResultsTab classes={classes} subjects={subjects} sessions={sessions} terms={termsForResultSession} students={students} resultFilters={resultFilters} updateResultFilter={updateResultFilter} selectedAssignment={selectedAssignment} resultForm={resultForm} setResultForm={setResultForm} saveResult={saveResult} editResult={editResult} resetResultForm={resetResultForm} editingResultId={editingResultId} visibleResults={visibleResults} updateResultStatus={updateResultStatus} isSaving={isSaving} />
      ) : null}

      {activeTab === "reportCards" ? (
        <ReportCardsTab classes={classes} sessions={sessions} terms={termsForReportSession} reportFilters={reportFilters} updateReportFilter={updateReportFilter} reportCardOverview={reportCardOverview} reportCards={visibleReportCards} generateReportCard={generateReportCard} regenerateReportCard={regenerateReportCard} publishReportCard={publishReportCard} isSaving={isSaving} />
      ) : null}
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-2">
      <Panel title={editingSessionId ? "Edit session" : "Create session"} subtitle="Manage academic years like 2026/2027.">
        <form onSubmit={saveSession} className="form-grid"><TextField label="Session" value={sessionForm.name} onChange={(value) => setSessionForm((c) => ({ ...c, name: value }))} placeholder="2026/2027" required /><TextField label="Start" type="date" value={sessionForm.start_date} onChange={(value) => setSessionForm((c) => ({ ...c, start_date: value }))} /><TextField label="End" type="date" value={sessionForm.end_date} onChange={(value) => setSessionForm((c) => ({ ...c, end_date: value }))} /><CheckboxField label="Current" checked={sessionForm.is_current} onChange={(value) => setSessionForm((c) => ({ ...c, is_current: value }))} /><FormActions editing={Boolean(editingSessionId)} onCancel={resetSessionForm} isSaving={isSaving === "session"} label={editingSessionId ? "Update session" : "Create session"} /></form>
        <DataTable headers={["Session", "Dates", "Status", "Actions"]} emptyText="No academic sessions yet." rows={sessions} renderRow={(session) => <tr key={session.id}><Cell label="Session"><strong>{session.name}</strong></Cell><Cell label="Dates">{compactDate(session.start_date)} - {compactDate(session.end_date)}</Cell><Cell label="Status"><StatusPill value={session.is_current ? "current" : session.is_active ? "active" : "inactive"} /></Cell><Cell label="Actions"><button type="button" className="btn-ghost" onClick={() => editSession(session)}><Edit3 className="h-3.5 w-3.5" /> Edit</button></Cell></tr>} />
      </Panel>

      <Panel title={editingTermId ? "Edit term" : "Create term"} subtitle="Terms are scoped to a session.">
        <form onSubmit={saveTerm} className="form-grid"><SelectField label="Session" value={termForm.academic_session_id} onChange={(value) => setTermForm((c) => ({ ...c, academic_session_id: value }))} required><option value="">Select session</option>{sessions.map((session) => <option key={session.id} value={session.id}>{session.name}</option>)}</SelectField><SelectField label="Term" value={termForm.name} onChange={(value) => setTermForm((c) => ({ ...c, name: value }))}><option value="first_term">First Term</option><option value="second_term">Second Term</option><option value="third_term">Third Term</option></SelectField><TextField label="Start" type="date" value={termForm.start_date} onChange={(value) => setTermForm((c) => ({ ...c, start_date: value }))} /><TextField label="End" type="date" value={termForm.end_date} onChange={(value) => setTermForm((c) => ({ ...c, end_date: value }))} /><CheckboxField label="Current" checked={termForm.is_current} onChange={(value) => setTermForm((c) => ({ ...c, is_current: value }))} /><FormActions editing={Boolean(editingTermId)} onCancel={resetTermForm} isSaving={isSaving === "term"} label={editingTermId ? "Update term" : "Create term"} /></form>
        <DataTable headers={["Term", "Session", "Dates", "Actions"]} emptyText="No academic terms yet." rows={terms} renderRow={(term) => <tr key={term.id}><Cell label="Term"><strong>{displayTerm(term.name)}</strong></Cell><Cell label="Session">{sessions.find((session) => session.id === term.academic_session_id)?.name || "--"}</Cell><Cell label="Dates">{compactDate(term.start_date)} - {compactDate(term.end_date)}</Cell><Cell label="Actions"><button type="button" className="btn-ghost" onClick={() => editTerm(term)}><Edit3 className="h-3.5 w-3.5" /> Edit</button></Cell></tr>} />
      </Panel>
      ...
      <Panel title={editingScaleId ? "Edit grading scale" : "Create grading scale"} subtitle="Grades are assigned only after all score components are filled.">
        <form onSubmit={saveScale} className="form-grid"><TextField label="Grade" value={scaleForm.grade} onChange={(value) => setScaleForm((c) => ({ ...c, grade: value }))} placeholder="A" required /><TextField label="Min" type="number" min="0" max="100" value={scaleForm.min_score} onChange={(value) => setScaleForm((c) => ({ ...c, min_score: value }))} required /><TextField label="Max" type="number" min="0" max="100" value={scaleForm.max_score} onChange={(value) => setScaleForm((c) => ({ ...c, max_score: value }))} required /><TextField label="Remark" value={scaleForm.remark} onChange={(value) => setScaleForm((c) => ({ ...c, remark: value }))} placeholder="Excellent" /><CheckboxField label="Active" checked={scaleForm.is_active} onChange={(value) => setScaleForm((c) => ({ ...c, is_active: value }))} /><FormActions editing={Boolean(editingScaleId)} onCancel={resetScaleForm} isSaving={isSaving === "scale"} label={editingScaleId ? "Update scale" : "Create scale"} /></form>
        <DataTable headers={["Grade", "Range", "Remark", "Actions"]} emptyText="No grading scales yet." rows={scales} renderRow={(scale) => <tr key={scale.id}><Cell label="Grade"><strong>{scale.grade}</strong></Cell><Cell label="Range">{scale.min_score} - {scale.max_score}</Cell><Cell label="Remark">{scale.remark || "--"}</Cell><Cell label="Actions"><button type="button" className="btn-ghost" onClick={() => editScale(scale)}><Edit3 className="h-3.5 w-3.5" /> Edit</button></Cell></tr>} />
      </Panel>
      ...
      <Panel title={editingSubjectId ? "Edit subject" : "Create subject"} subtitle="Subjects are tenant-wide. Case and spacing duplicates are blocked by the backend.">
        <form onSubmit={saveSubject} className="form-grid"><TextField label="Name" value={subjectForm.name} onChange={(value) => setSubjectForm((c) => ({ ...c, name: value }))} placeholder="Biology" required /><TextField label="Code" value={subjectForm.code} onChange={(value) => setSubjectForm((c) => ({ ...c, code: value }))} placeholder="BIO" /><TextField label="Description" value={subjectForm.description} onChange={(value) => setSubjectForm((c) => ({ ...c, description: value }))} placeholder="Optional" /><FormActions editing={Boolean(editingSubjectId)} onCancel={resetSubjectForm} isSaving={isSaving === "subject"} label={editingSubjectId ? "Update subject" : "Create subject"} /></form>
        <DataTable headers={["Subject", "Code", "Status", "Actions"]} emptyText="No subjects yet." rows={subjects} renderRow={(subject) => <tr key={subject.id}><Cell label="Subject"><strong>{subject.name}</strong></Cell><Cell label="Code">{subject.code || "--"}</Cell><Cell label="Status"><StatusPill value={subject.is_active === false ? "inactive" : "active"} /></Cell><Cell label="Actions"><button type="button" className="btn-ghost" onClick={() => editSubject(subject)}><Edit3 className="h-3.5 w-3.5" /> Edit</button></Cell></tr>} />
      </Panel>
    </div>
  );
}

function AssignmentsTab({ classes, subjects, teachers, classSubjects, assignmentFilters, setAssignmentFilters, assignmentForm, setAssignmentForm, visibleAssignments, saveAssignment, editAssignment, resetAssignmentForm, editingAssignmentId, deactivateAssignment, isSaving }) {
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.25fr)]">
      <Panel title={editingAssignmentId ? "Change teacher" : "Assign subject teacher"} subtitle="Select a class, choose a subject, then assign the teacher who teaches it.">
        <form onSubmit={saveAssignment} className="grid gap-3">
          <SelectField label="Class" value={assignmentForm.class_id} onChange={(value) => setAssignmentForm((c) => ({ ...c, class_id: value, class_subject_id: "" }))} required><option value="">Select class</option>{classes.map((item) => <option key={item.id} value={item.id}>{displayClass(item)}</option>)}</SelectField>
          <SelectField label="Subject" value={assignmentForm.class_subject_id} onChange={(value) => setAssignmentForm((c) => ({ ...c, class_subject_id: value }))} disabled={!assignmentForm.class_id} required><option value="">Select subject</option>{classSubjects.map((item) => <option key={item.id} value={item.id}>{subjectLabel(item)}{item.is_offered_by_class === false ? " · add to class" : ""}</option>)}</SelectField>
          <SelectField label="Teacher" value={assignmentForm.teacher_id} onChange={(value) => setAssignmentForm((c) => ({ ...c, teacher_id: value }))} required><option value="">Select teacher</option>{teachers.map((teacher) => <option key={teacher.id} value={teacher.id}>{teacherLabel(teacher)}{teacher.staff_id ? ` · ${teacher.staff_id}` : ""}</option>)}</SelectField>
          <CheckboxField label="Core subject" checked={assignmentForm.is_core} onChange={(value) => setAssignmentForm((c) => ({ ...c, is_core: value }))} disabled={Boolean(editingAssignmentId)} helperText={editingAssignmentId ? "Only the teacher changes during reassignment." : "Used if this subject is being added to the class."} />
      ...
      <Panel title="Teacher assignment map" subtitle="Old assignments should be inactive, not deleted.">
        <AssignmentFilterGrid filters={assignmentFilters} setFilters={setAssignmentFilters} classes={classes} subjects={subjects} teachers={teachers} />
        <DataTable headers={["Class", "Subject", "Teacher", "Status", "Actions"]} emptyText="No assignments match these filters." rows={visibleAssignments} renderRow={(assignment) => <tr key={assignment.id}><Cell label="Class"><strong>{classLabel(assignment)}</strong></Cell><Cell label="Subject">{subjectLabel(assignment)}</Cell><Cell label="Teacher">{assignment.teacher_name || assignment.teacher_staff_id || "--"}</Cell><Cell label="Status"><StatusPill value={assignment.is_active ? "active" : "inactive"} /></Cell><Cell label="Actions"><ActionRow><button type="button" className="btn-ghost" onClick={() => editAssignment(assignment)}><Edit3 className="h-3.5 w-3.5" /> Change</button>{assignment.is_active ? <button type="button" className="btn-deact" disabled={isSaving === assignment.id} onClick={() => deactivateAssignment(assignment)}>Deactivate</button> : null}</ActionRow></Cell></tr>} />
      </Panel>
    </div>
  );
}

function ResultsTab({ classes, subjects, sessions, terms, students, resultFilters, updateResultFilter, selectedAssignment, resultForm, setResultForm, saveResult, editResult, resetResultForm, editingResultId, visibleResults, updateResultStatus, isSaving }) {
  const submitBlocked = resultForm.status === "submitted" && (!hasAllScores(resultForm) || scoreTotal(resultForm) > 100);
  return (
    <div className="space-y-5">
      <Panel title="Result filters" subtitle="Choose a class-subject first. The student dropdown loads from the selected class.">
        <ResultFilterGrid filters={resultFilters} updateFilter={updateResultFilter} classes={classes} subjects={subjects} sessions={sessions} terms={terms} />
        {selectedAssignment ? <AssignmentSummary assignment={selectedAssignment} /> : resultFilters.class_id && resultFilters.subject_id ? <Alert tone="warning">No active teacher assignment found for this class-subject. Assign a teacher before saving scores.</Alert> : null}
      </Panel>
      <Panel title={editingResultId ? "Correct result" : "Create result"} subtitle="Use blank fields for missing scores. Grades appear only after all three score components are filled.">
        <form onSubmit={saveResult} className="form-grid">
          <SelectField label="Student" value={resultForm.student_id} onChange={(value) => setResultForm((c) => ({ ...c, student_id: value }))} disabled={!resultFilters.class_id} required><option value="">{resultFilters.class_id ? "Select student" : "Select class first"}</option>{students.map((student) => <option key={student.id} value={student.id}>{studentLabel(student)}{student.admission_number ? ` · ${student.admission_number}` : ""}</option>)}</SelectField>
          <TextField label="Test" type="number" min="0" max="100" value={resultForm.test_score} onChange={(value) => setResultForm((c) => ({ ...c, test_score: value }))} />
          <TextField label="Assessment" type="number" min="0" max="100" value={resultForm.assessment_score} onChange={(value) => setResultForm((c) => ({ ...c, assessment_score: value }))} />
          <TextField label="Exam" type="number" min="0" max="100" value={resultForm.exam_score} onChange={(value) => setResultForm((c) => ({ ...c, exam_score: value }))} />
          <SelectField label="Status" value={resultForm.status} onChange={(value) => setResultForm((c) => ({ ...c, status: value }))}><option value="draft">Draft</option><option value="submitted">Submitted</option></SelectField>
          <FormActions editing={Boolean(editingResultId)} onCancel={resetResultForm} isSaving={isSaving === "result"} disabled={!selectedAssignment || submitBlocked} label={editingResultId ? "Save correction" : "Save result"} />
        </form>
        {submitBlocked ? <p className="mt-3 text-xs font-semibold text-amber-600">Submitted results require all three scores and a combined total not above 100.</p> : null}
      </Panel>
      <Panel title="Results" subtitle="Submitted scores feed report cards. Draft scores remain internal.">
        <DataTable headers={["Student", "Subject", "Scores", "Total", "Grade", "Status", "Actions"]} emptyText="No results match these filters." rows={visibleResults} renderRow={(result) => <tr key={result.id}><Cell label="Student"><strong>{result.student_name || result.admission_number || "Student"}</strong></Cell><Cell label="Subject">{subjectLabel(result)}</Cell><Cell label="Scores">{displayScore(result.test_score)} / {displayScore(result.assessment_score)} / {displayScore(result.exam_score)}</Cell><Cell label="Total">{displayScore(result.total_score)}</Cell><Cell label="Grade">{result.grade || "--"}</Cell><Cell label="Status"><StatusPill value={result.status} /></Cell><Cell label="Actions"><ActionRow><button type="button" className="btn-ghost" onClick={() => editResult(result)}><Edit3 className="h-3.5 w-3.5" /> {isSubmitted(result) ? "Correct" : "Edit"}</button><button type="button" className="btn-ghost" disabled={isSaving === result.id} onClick={() => updateResultStatus(result, isSubmitted(result) ? "draft" : "submitted")}>{isSubmitted(result) ? "Reopen draft" : "Submit"}</button></ActionRow></Cell></tr>} />
      </Panel>
    </div>
  );
}

function ReportCardsTab({ classes, sessions, terms, reportFilters, updateReportFilter, reportCardOverview, reportCards, generateReportCard, regenerateReportCard, publishReportCard, isSaving }) {
  return (
    <div className="space-y-5">
...
function ResultFilterGrid({ filters, updateFilter, classes = [], subjects = [], sessions = [], terms = [], hideSubject = false }) {
  return <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><SelectField label="Class" value={filters.class_id} onChange={(value) => updateFilter("class_id", value)}><option value="">Select class</option>{classes.map((item) => <option key={item.id} value={item.id}>{displayClass(item)}</option>)}</SelectField>{!hideSubject ? <SelectField label="Subject" value={filters.subject_id} onChange={(value) => updateFilter("subject_id", value)}><option value="">Select subject</option>{subjects.map((item) => <option key={item.id} value={item.id}>{item.name || item.code || "Subject"}</option>)}</SelectField> : null}<SelectField label="Session" value={filters.academic_session_id} onChange={(value) => updateFilter("academic_session_id", value)}><option value="">Select session</option>{sessions.map((item) => <option key={item.id} value={item.id}>{item.name}{item.is_current ? " · Current" : ""}</option>)}</SelectField><SelectField label="Term" value={filters.academic_term_id} onChange={(value) => updateFilter("academic_term_id", value)}><option value="">Select term</option>{terms.map((item) => <option key={item.id} value={item.id}>{displayTerm(item.name)}{item.is_current ? " · Current" : ""}</option>)}</SelectField></div>;
}

function AssignmentSummary({ assignment }) {
  return <div className="mt-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-text-soft"><p className="font-semibold text-text">Recording for {assignment.subject_name || "subject"} in {classLabel(assignment)}</p><p className="mt-1">Active teacher: {assignment.teacher_name || assignment.teacher_staff_id || "Teacher assigned"}</p></div>;
}
