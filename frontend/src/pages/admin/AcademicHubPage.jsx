...
import {
  ...
  displayPerson,
  displayTerm,
} from "../../components/academic/academicDisplay";
...
const tabs = [
  {
    ...
    label: "Setup",
    icon: BookOpen,
    description: "Sessions, terms, grading scales, and subjects",
  },
  {
    id: "assignments",
    label: "Teacher Assignments",
    icon: Users,
    description: "Assign subject teachers to class subjects",
  },
  {
    id: "results",
    label: "Results",
    icon: Pencil,
    description: "Record, correct, submit, and reopen scores",
  },
  {
    id: "reportCards",
    label: "Report Cards",
    icon: FileText,
    description: "Generate, regenerate, publish, and print",
  },
  {
    id: "search",
    label: "Search",
    icon: Search,
    description: "Find academic records quickly",
  },
];
...
const subjectLabel = (item) =>
  item?.subject_name || item?.name || item?.subject_code || item?.code || "Subject";
const classLabel = (item) =>
  [item?.class_name || item?.name, item?.class_arm || item?.arm]
    .filter(Boolean)
    .join(" ") || "Class";
const teacherLabel = (teacher) =>
  displayPerson(teacher) ||
  teacher?.teacher_name ||
  teacher?.teacher_staff_id ||
  "Teacher";
const studentLabel = (student) =>
  ...
  "Student";
const scoreTotal = (form) =>
  [form.test_score, form.assessment_score, form.exam_score].reduce(
    (sum, value) => sum + (Number(value) || 0),
    0
  );
const hasAllScores = (form) =>
  [form.test_score, form.assessment_score, form.exam_score].every(
    (value) => value !== "" && value !== null && value !== undefined
  );
const studentClassId = (student) =>
  student?.class_id ||
...
function AcademicHubPage() {
  ...
  const assignmentSubject = classSubjects.find(
    (item) => item.id === assignmentForm.class_subject_id
  );
  ...
  const termsForResultSession = useMemo(
    () =>
      terms.filter(
        (term) =>
          !resultFilters.academic_session_id ||
          term.academic_session_id === resultFilters.academic_session_id
      ),
    ...
    () =>
      terms.filter(
        (term) =>
          ...
          term.academic_session_id === reportFilters.academic_session_id
      ),
  ...
  const visibleAssignments = useMemo(
    () =>
      assignments.filter(
        (item) =>
          (!assignmentFilters.class_id ||
            item.class_id === assignmentFilters.class_id) &&
          (!assignmentFilters.subject_id ||
            item.subject_id === assignmentFilters.subject_id) &&
          (!assignmentFilters.teacher_id ||
            item.teacher_id === assignmentFilters.teacher_id)
      ),
  ...
  const selectedAssignment = useMemo(() => {
    ...
    return assignments.find(
      (item) =>
        item.is_active &&
        item.class_id === resultFilters.class_id &&
        item.subject_id === resultFilters.subject_id
    );
  ...
  const visibleResults = useMemo(
    () =>
      results.filter(
        (item) => !resultFilters.subject_id || item.subject_id === resultFilters.subject_id
      ),
  ...
  const visibleReportCards = useMemo(
    () =>
      reportCards.filter(
        (card) =>
          (!reportFilters.academic_session_id ||
            card.academic_session_id === reportFilters.academic_session_id) &&
          (!reportFilters.academic_term_id ||
            card.academic_term_id === reportFilters.academic_term_id) &&
  ...
  const loadWorkspace = async () => {
    ...
    const defaultSessionId =
      nextSessions.find((item) => item.is_current)?.id || nextSessions[0]?.id || "";
    const defaultTermId =
      nextTerms.find((item) => item.is_current)?.id || nextTerms[0]?.id || "";
  ...
  useEffect(() => {
    ...
    async function loadClassSubjects() {
      ...
      try {
        const response = await academicService.listClassSubjects(
  ...
  }, [assignmentForm.class_id, showError]);
    ...
    async function loadStudentsForClass() {
      ...
      try {
        const response = await studentService.getAdminStudents({
          classId: resultFilters.class_id,
          limit: 100,
        });
        const items = (response?.items || []).filter(
          (student) => studentClassId(student) === resultFilters.class_id
        );
  ...
  useEffect(() => {
    ...
    async function loadFilteredResults() {
      if (
        ...
        !resultFilters.academic_session_id ||
        !resultFilters.academic_term_id
      ) {
        setResults([]);
        return;
      }
  ...
  const resetTermForm = () => {
    setTermForm({
      ...blankTerm,
      academic_session_id: resultFilters.academic_session_id || currentSession?.id || "",
    });
    setEditingTermId("");
  };
  ...
  const updateResultFilter = (key, value) => {
    setResultFilters((current) => ({
      ...current,
      [key]: value,
      ...(key === "academic_session_id" ? { academic_term_id: "" } : {}),
    }));
    resetResultForm();
  };
  ...
  const saveAssignment = async (event) => {
    ...
    try {
      ...
      if (assignmentForm.class_id) {
        const classSubjectResponse = await academicService.listClassSubjects(
          assignmentForm.class_id,
          { active_only: true }
        );
      ...
      setAssignmentFilters({
        class_id: saved.class_id || assignmentForm.class_id,
        subject_id: saved.subject_id || assignmentSubject?.subject_id || "",
        teacher_id: saved.teacher_id || assignmentForm.teacher_id,
      });
      resetAssignmentForm();
  ...
  const activateAssignment = async (assignment) => {
    setIsSaving(assignment.id);
    try {
      const saved = await academicService.activateTeacherAssignment(assignment.id);
      setAssignments((current) => current.map((item) => (item.id === saved.id ? saved : item)));
      showSuccess("Assignment reactivated.");
    } catch (err) {
      showError(getErrorMessage(err, "Could not reactivate assignment."));
    } finally {
      setIsSaving("");
    }
  };
  ...
  const saveResult = async (event) => {
    event.preventDefault();
    const assignmentId =
      editingResultId && resultForm.teacher_assignment_id
        ? resultForm.teacher_assignment_id
  ...
  const updateResultStatus = async (result, nextStatus) => {
    ...
    try {
      const saved = await academicService.updateResultStatus(result.id, {
        status: nextStatus,
      });
  ...
  const generateReportCard = async (payload, label) => {
    ...
    try {
      ...
      if (Array.isArray(response?.generated)) {
        ...
        showSuccess(
          `Generated ${response.generated.length} report cards. Skipped ${response.skipped?.length || 0}.`
        );
      } else {
  ...
  return (
    <DashboardLayout
      role="admin"
      title="Academic Hub"
      description="Configure academic setup, teacher assignments, results, and official report cards."
    >
      ...
      <section className="rounded-[1.35rem] border border-border bg-surface p-4 shadow-sm sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs sm:min-w-[280px]">
            ...
            <Context
              label="Current Term"
              value={currentTerm?.name ? displayTerm(currentTerm.name) : "Not set"}
            />
          </div>
        </div>
      </section>
    ...
    <div className="grid gap-5 2xl:grid-cols-2">
      <Panel title={editingSessionId ? "Edit session" : "Create session"} subtitle="Manage academic years like 2026/2027.">
        <form onSubmit={saveSession} className="form-grid">
          ...
          <TextField label="Start" type="date" value={sessionForm.start_date} onChange={(value) => setSessionForm((c) => ({ ...c, start_date: value }))} />
          <TextField label="End" type="date" value={sessionForm.end_date} onChange={(value) => setSessionForm((c) => ({ ...c, end_date: value }))} />
      ...
      <Panel title={editingTermId ? "Edit term" : "Create term"} subtitle="Terms are scoped to a session.">
        <form onSubmit={saveTerm} className="form-grid">
          ...
          <SelectField label="Term" value={termForm.name} onChange={(value) => setTermForm((c) => ({ ...c, name: value }))} required>
            <option value="first_term">First term</option>
            <option value="second_term">Second term</option>
            <option value="third_term">Third term</option>
          </SelectField>
          <TextField label="Start" type="date" value={termForm.start_date} onChange={(value) => setTermForm((c) => ({ ...c, start_date: value }))} />
          <TextField label="End" type="date" value={termForm.end_date} onChange={(value) => setTermForm((c) => ({ ...c, end_date: value }))} />
        ...
        <DataTable headers={["Term", "Session", "Dates", "Status", "Actions"]} emptyText="No academic terms yet." rows={terms} renderRow={(term) => (
          <tr key={term.id}>
            <Cell label="Term"><strong>{displayTerm(term.name)}</strong></Cell>
            <Cell label="Session">{term.academic_session_name || sessions.find((session) => session.id === term.academic_session_id)?.name || "--"}</Cell>
            <Cell label="Dates">{compactDate(term.start_date)} - {compactDate(term.end_date)}</Cell>
            <Cell label="Status"><StatusPill value={term.is_current ? "current" : term.is_active ? "active" : "inactive"} /></Cell>
      ...
      <Panel title={editingScaleId ? "Edit grading scale" : "Create grading scale"} subtitle="Define grade bands for report cards.">
        <form onSubmit={saveScale} className="form-grid">
          ...
          <TextField label="Min score" type="number" min="0" max="100" value={scaleForm.min_score} onChange={(value) => setScaleForm((c) => ({ ...c, min_score: value }))} required />
          <TextField label="Max score" type="number" min="0" max="100" value={scaleForm.max_score} onChange={(value) => setScaleForm((c) => ({ ...c, max_score: value }))} required />
        ...
        <DataTable headers={["Grade", "Range", "Remark", "Status", "Actions"]} emptyText="No grading scales yet." rows={scales} renderRow={(scale) => (
          <tr key={scale.id}>
            ...
            <Cell label="Status"><StatusPill value={scale.is_active ? "active" : "inactive"} /></Cell>
      ...
      <Panel title={editingSubjectId ? "Edit subject" : "Create subject"} subtitle="Maintain the subject catalogue used by classes and report cards.">
        <form onSubmit={saveSubject} className="form-grid">
          <TextField label="Subject name" value={subjectForm.name} onChange={(value) => setSubjectForm((c) => ({ ...c, name: value }))} placeholder="Mathematics" required />
          <TextField label="Code" value={subjectForm.code} onChange={(value) => setSubjectForm((c) => ({ ...c, code: value }))} placeholder="MTH" />
          <TextField label="Description" value={subjectForm.description} onChange={(value) => setSubjectForm((c) => ({ ...c, description: value }))} placeholder="Optional description" />
        ...
        <DataTable headers={["Subject", "Code", "Description", "Actions"]} emptyText="No subjects yet." rows={subjects} renderRow={(subject) => (
          <tr key={subject.id}>
            <Cell label="Subject"><strong>{subject.name}</strong></Cell>
            <Cell label="Code">{subject.code || "--"}</Cell>
            <Cell label="Description">{subject.description || "--"}</Cell>
            <Cell label="Actions"><button type="button" className="btn-ghost" onClick={() => editSubject(subject)}><Edit3 className="h-3.5 w-3.5" /> Edit</button></Cell>
          </tr>
        )} />
      </Panel>
    </div>
...
function AssignmentsTab({
  ...
  return (
    <div className="space-y-5">
      <Panel title={editingAssignmentId ? "Change teacher assignment" : "Assign subject teacher"} subtitle="Choose a class, class subject, and teacher. Existing assignments remain as history.">
        <form onSubmit={saveAssignment} className="form-grid">
          <SelectField label="Class" value={assignmentForm.class_id} onChange={(value) => setAssignmentForm((c) => ({ ...c, class_id: value, class_subject_id: "" }))} required>
            <option value="">Select class</option>
            {classes.map((item) => <option key={item.id} value={item.id}>{displayClass(item)}</option>)}
          </SelectField>
          <SelectField label="Class subject" value={assignmentForm.class_subject_id} onChange={(value) => setAssignmentForm((c) => ({ ...c, class_subject_id: value }))} disabled={!assignmentForm.class_id} required>
            <option value="">{assignmentForm.class_id ? "Select subject" : "Select class first"}</option>
            {classSubjects.map((item) => <option key={item.id} value={item.id}>{subjectLabel(item)}</option>)}
          </SelectField>
          <SelectField label="Teacher" value={assignmentForm.teacher_id} onChange={(value) => setAssignmentForm((c) => ({ ...c, teacher_id: value }))} required>
            <option value="">Select teacher</option>
            {teachers.map((item) => <option key={item.id} value={item.id}>{teacherLabel(item)}</option>)}
          </SelectField>
          <CheckboxField label="Core subject" checked={assignmentForm.is_core} onChange={(value) => setAssignmentForm((c) => ({ ...c, is_core: value }))} />
      ...
      <Panel title="Teacher assignments" subtitle="Filter assignments by class, subject, or teacher.">
        ...
        <DataTable headers={["Class", "Subject", "Teacher", "Status", "Actions"]} emptyText="No teacher assignments match these filters." rows={visibleAssignments} renderRow={(assignment) => (
          <tr key={assignment.id}>
            <Cell label="Class">{classLabel(assignment)}</Cell>
            <Cell label="Subject">{subjectLabel(assignment)}</Cell>
            <Cell label="Teacher"><strong>{assignment.teacher_name || assignment.teacher_staff_id || "Teacher"}</strong></Cell>
            ...
            <Cell label="Actions">
              <ActionRow>
                ...
                {assignment.is_active ? (
                  <button type="button" className="btn-deact" disabled={isSaving === assignment.id} onClick={() => deactivateAssignment(assignment)}>Deactivate</button>
                ) : (
                  <button type="button" className="btn-create" disabled={isSaving === assignment.id} onClick={() => activateAssignment(assignment)}><Plus className="h-3.5 w-3.5" /> Activate</button>
                )}
              </ActionRow>
            </Cell>
          </tr>
...
function ResultsTab({
  ...
  const submitBlocked =
    resultForm.status === "submitted" &&
    (!hasAllScores(resultForm) || scoreTotal(resultForm) > 100);
  ...
  return (
    <div className="space-y-5">
      <Panel title="Result filters" subtitle="Choose a class-subject first. The student dropdown loads from the selected class.">
        ...
        ) : resultFilters.class_id && resultFilters.subject_id ? (
      ...
      <Panel title={editingResultId ? "Correct result" : "Create result"} subtitle="Use blank fields for missing scores. Grades appear only after all three score components are filled.">
        <form onSubmit={saveResult} className="form-grid">
          <SelectField label="Student" value={resultForm.student_id} onChange={(value) => setResultForm((c) => ({ ...c, student_id: value }))} disabled={!resultFilters.class_id} required>
            <option value="">{resultFilters.class_id ? "Select student" : "Select class first"}</option>
          ...
          <SelectField label="Status" value={resultForm.status} onChange={(value) => setResultForm((c) => ({ ...c, status: value }))}>
          ...
          <tr key={result.id}>
            ...
            <Cell label="Actions">
              <ActionRow>
                ...
                <button type="button" className="btn-ghost" disabled={isSaving === result.id} onClick={() => updateResultStatus(result, isSubmitted(result) ? "draft" : "submitted")}>{isSubmitted(result) ? "Reopen draft" : "Submit"}</button>
              </ActionRow>
            </Cell>
          </tr>
        )} />
  ...
  return (
    <div className="space-y-5">
      <Panel title="Report card filters" subtitle="Report cards are official snapshots generated after submitted scores are complete.">
        <ResultFilterGrid filters={reportFilters} updateFilter={updateReportFilter} classes={classes} sessions={sessions} terms={terms} hideSubject />
        <div className="mt-4 flex flex-col gap-2 sm:flex-row">
          <button type="button" className="btn-create" disabled={!reportFilters.class_id || !reportFilters.academic_session_id || !reportFilters.academic_term_id || isSaving === "bulk-report"} onClick={() => generateReportCard({ class_id: reportFilters.class_id, academic_session_id: reportFilters.academic_session_id, academic_term_id: reportFilters.academic_term_id }, "bulk-report")}>
            <Plus className="h-3.5 w-3.5" /> Bulk generate for class
          </button>
        </div>
      </Panel>

      <Panel title="Completion overview" subtitle="Generate one student when submitted score count matches expected class subjects.">
          ...
          return (
            <tr key={row.student_id}>
              ...
              <Cell label="Actions">
                <ActionRow>
                  {!row.report_card_id ? (
                    <button type="button" className="btn-create" disabled={!complete || isSaving === row.student_id} onClick={() => generateReportCard({ student_id: row.student_id, academic_session_id: reportFilters.academic_session_id, academic_term_id: reportFilters.academic_term_id }, row.student_id)}>Generate</button>
                  ) : null}
                  {row.report_card_id ? (
                    <button type="button" className="btn-ghost" onClick={() => reportCardService.printAdminReportCard(row.report_card_id)}>Print</button>
                  ) : null}
                  {row.report_card_id ? (
                    <button type="button" className="btn-ghost" disabled={isSaving === row.report_card_id} onClick={() => regenerateReportCard(row.report_card_id)}>Regenerate</button>
                  ) : null}
                </ActionRow>
              </Cell>
            </tr>
            ...
            <Cell label="Actions">
              <ActionRow>
                ...
                {card.status !== "published" ? (
                  <button type="button" className="btn-create" disabled={isSaving === card.id} onClick={() => publishReportCard(card.id)}>Publish</button>
                ) : null}
              </ActionRow>
            </Cell>
          </tr>
...
function AssignmentFilterGrid({ filters, setFilters, classes = [], subjects = [], teachers = [] }) {
  return (
    <div className="mb-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      <SelectField label="Class" value={filters.class_id} onChange={(value) => setFilters((c) => ({ ...c, class_id: value }))}>
        <option value="">All classes</option>
        {classes.map((item) => <option key={item.id} value={item.id}>{displayClass(item)}</option>)}
      </SelectField>
      <SelectField label="Subject" value={filters.subject_id} onChange={(value) => setFilters((c) => ({ ...c, subject_id: value }))}>
        <option value="">All subjects</option>
        {subjects.map((item) => <option key={item.id} value={item.id}>{item.name || item.code || "Subject"}</option>)}
      </SelectField>
      <SelectField label="Teacher" value={filters.teacher_id} onChange={(value) => setFilters((c) => ({ ...c, teacher_id: value }))}>
...
function ResultFilterGrid({ filters, updateFilter, classes = [], subjects = [], sessions = [], terms = [], hideSubject = false }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <SelectField label="Class" value={filters.class_id} onChange={(value) => updateFilter("class_id", value)}>
        <option value="">Select class</option>
        {classes.map((item) => <option key={item.id} value={item.id}>{displayClass(item)}</option>)}
      </SelectField>
      {!hideSubject ? (
        <SelectField label="Subject" value={filters.subject_id} onChange={(value) => updateFilter("subject_id", value)}>
          <option value="">Select subject</option>
          {subjects.map((item) => <option key={item.id} value={item.id}>{item.name || item.code || "Subject"}</option>)}
        </SelectField>
      ) : null}
      <SelectField label="Session" value={filters.academic_session_id} onChange={(value) => updateFilter("academic_session_id", value)}>
        <option value="">Select session</option>
        {sessions.map((item) => <option key={item.id} value={item.id}>{item.name}{item.is_current ? " · Current" : ""}</option>)}
      </SelectField>
      <SelectField label="Term" value={filters.academic_term_id} onChange={(value) => updateFilter("academic_term_id", value)}>
...
function AssignmentSummary({ assignment }) {
  return (
    <div className="mt-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-text-soft">
      <p className="font-semibold text-text">Recording for {assignment.subject_name || "subject"} in {classLabel(assignment)}</p>
      <p className="mt-1">Active teacher: {assignment.teacher_name || assignment.teacher_staff_id || "Teacher assigned"}</p>
    </div>
  );
}
...
function Metric({ label, value }) {
  return (
    <div className="h-full rounded-2xl border border-border bg-surface px-3 py-3 shadow-sm sm:px-4">
      <p className="text-[10px] font-semibold uppercase tracking-wide text-text-muted sm:text-[11px]">{label}</p>
      <p className="mt-1 text-2xl font-semibold leading-none text-text sm:text-[1.9rem]">{value}</p>
    </div>
  );
}
...
function Alert({ children, tone = "info" }) {
  ...
  return <div className={`mt-4 rounded-2xl border px-4 py-3 text-sm font-medium ${styles}`}>{children}</div>;
}
