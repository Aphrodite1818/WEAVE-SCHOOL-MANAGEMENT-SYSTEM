...
Place in ``backend/scripts/seed_full_dashboard_demo_data.py`` and run:

    python scripts/seed_full_dashboard_demo_data.py --reset --seed 20260727

The script automatically looks for the existing single-school seeder in the same
directory. Pass ``--base-seeder`` only when that seeder lives elsewhere.
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional path to the existing single-school Faker seeder. "
            "When omitted, the script searches the current scripts directory."
        ),
    )
    ...
    if args.base_seeder is not None:
        base_seeder_path = args.base_seeder.expanduser().resolve()
    else:
        scripts_dir = Path(__file__).resolve().parent
        current_file = Path(__file__).resolve()
        candidates = (
            scripts_dir / "seed_full_dashboard_data.py",
            scripts_dir / "seed_dashboard_data.py",
            scripts_dir / "seed_full_dashboard.py",
            scripts_dir / "seed_data.py",
        )
        base_seeder_path = next(
            (candidate for candidate in candidates if candidate.exists() and candidate.resolve() != current_file),
            None,
        )
        if base_seeder_path is None:
            searched = "\n  - ".join(str(path) for path in candidates)
            parser.error(
                "Could not locate the existing single-school seeder automatically. "
                "Expected one of:\n  - "
                f"{searched}\n"
                "Add the original seeder to backend/scripts or pass its path with "
                "--base-seeder."
            )

    print(f"Using base seeder: {base_seeder_path}")
    base_seeder = load_base_seeder(base_seeder_path)
