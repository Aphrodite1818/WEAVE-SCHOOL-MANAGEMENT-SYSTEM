...
ALTER TABLE public.classes
DROP CONSTRAINT IF EXISTS uq_classes_tenant_normalized_name_arm;
...
ALTER TABLE public.classes
DROP COLUMN IF EXISTS normalized_arm;
...
def execute_sql_block(sql_block: str) -> None:
    """Execute semicolon-separated SQL statements one at a time for asyncpg."""
...
def upgrade() -> None:
    execute_sql_block(NORMALIZE_HELPERS_SQL)
    execute_sql_block(UPGRADE_SQL)
    execute_sql_block(DROP_HELPERS_SQL)


def downgrade() -> None:
    execute_sql_block(NORMALIZE_HELPERS_SQL)
    execute_sql_block(DOWNGRADE_SQL)
    execute_sql_block(DROP_HELPERS_SQL)
