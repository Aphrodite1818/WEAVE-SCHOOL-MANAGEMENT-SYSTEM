...
class CloudflareR2MediaStorage:
    ...
    def __init__(self) -> None:
        self.account_id = self._require_setting("CLOUDFLARE_R2_ACCOUNT_ID")
        self.access_key_id = self._require_setting("CLOUDFLARE_R2_ACCESS_KEY_ID")
        self.secret_access_key = self._require_setting("CLOUDFLARE_R2_SECRET_ACCESS_KEY")
        self.bucket = self._require_setting("CLOUDFLARE_R2_BUCKET_NAME")

        self.public_base_url = self._optional_setting("CLOUDFLARE_R2_PUBLIC_BASE_URL")
        self.cdn_base_url = self._optional_setting("CLOUDFLARE_R2_CDN_BASE_URL")
    ...
    def _build_public_url(self, object_key: str) -> str | None:
        """Build public URL if a public base URL is configured."""
    ...
    def _build_cdn_url(self, object_key: str) -> str | None:
        """Build CDN URL if a CDN base URL is configured."""
    ...
    async def upload_object(
        ...
        *,
        ...
        if visibility == MediaVisibility.PUBLIC:
            public_url = self._build_public_url(cleaned_key)
            cdn_url = self._build_cdn_url(cleaned_key)
    ...
    async def create_signed_url(
        ...
        *,
        ...
        try:
            ...
            raise CloudflareR2StorageError("Failed to create signed URL for Cloudflare R2 object.") from exc