...
from app.modules.classes.schemas import AcademicCategoryOption, AcademicLevelCreate, AcademicLevelResponse, AcademicLevelUpdate
...
async def create_academic_level(payload: AcademicLevelCreate, db: DbSession, current_user: CurrentTenantAdmin):
    return await AcademicLevelService.create(db, current_user, payload)

@router.get("", response_model=list[AcademicLevelResponse])
async def list_academic_levels(db: DbSession, current_user: CurrentTenantMember, active_only: bool = Query(default=False), include_archived: bool = Query(default=False)):
    return await AcademicLevelService.list(db, current_user, active_only=active_only, include_archived=include_archived)

@router.patch("/{academic_level_id}", response_model=AcademicLevelResponse)
async def update_academic_level(academic_level_id: uuid.UUID, payload: AcademicLevelUpdate, db: DbSession, current_user: CurrentTenantAdmin):
