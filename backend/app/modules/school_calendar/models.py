...
class SchoolCalendarConfiguration(BaseModel):
    ...
    timezone: Mapped[str] = mapped_column(String(80), nullable=False, default="Africa/Lagos", server_default="Africa/Lagos")
    ...
    __table_args__ = (
        ...
        CheckConstraint("cardinality(instructional_weekdays) > 0", name="ck_school_calendar_config_weekdays_not_empty"),
        CheckConstraint("instructional_weekdays <@ ARRAY[0,1,2,3,4,5,6]", name="ck_school_calendar_config_weekdays_range"),
        CheckConstraint(
...
class SchoolCalendar(BaseModel):
    ...
    status: Mapped[SchoolCalendarStatus] = mapped_column(
        SQLEnum(SchoolCalendarStatus, name="school_calendar_status", schema=PUBLIC_SCHEMA, values_callable=enum_values),
        nullable=False,
    ...
    archived_by_admin_id: Mapped[uuid.UUID | None] = mapped_column(UUID, ForeignKey("tenant_admins.id", ondelete="SET NULL"), nullable=True)
...
class SchoolCalendarDay(BaseModel):
    ...
    calendar_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("school_calendars.id", ondelete="RESTRICT"), nullable=False, index=True)
    academic_session_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("academic_sessions.id", ondelete="RESTRICT"), nullable=False, index=True)
    academic_term_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("academic_terms.id", ondelete="RESTRICT"), nullable=False, index=True)
    calendar_date: Mapped[date] = mapped_column(Date, nullable=False)
    day_type: Mapped[SchoolCalendarDayType] = mapped_column(
        SQLEnum(SchoolCalendarDayType, name="school_calendar_day_type", schema=PUBLIC_SCHEMA, values_callable=enum_values),
        nullable=False,
    )
    ...
    source: Mapped[SchoolCalendarDaySource] = mapped_column(
        SQLEnum(SchoolCalendarDaySource, name="school_calendar_day_source", schema=PUBLIC_SCHEMA, values_callable=enum_values),
        nullable=False,
    ...
    created_by_admin_id: Mapped[uuid.UUID | None] = mapped_column(UUID, ForeignKey("tenant_admins.id", ondelete="SET NULL"), nullable=True)
    updated_by_admin_id: Mapped[uuid.UUID | None] = mapped_column(UUID, ForeignKey("tenant_admins.id", ondelete="SET NULL"), nullable=True)

    __table_args__ = (
        UniqueConstraint("tenant_id", "calendar_id", "calendar_date", name="uq_school_calendar_days_tenant_calendar_date"),
        Index("ix_school_calendar_days_tenant_date", "tenant_id", "calendar_date"),
        Index("ix_school_calendar_days_tenant_term_date", "tenant_id", "academic_term_id", "calendar_date"),
        CheckConstraint("opens_at IS NULL OR closes_at IS NULL OR closes_at > opens_at", name="ck_school_calendar_days_open_close_order"),
        CheckConstraint("student_attendance_required = false OR student_activity_allowed = true", name="ck_school_calendar_days_attendance_requires_activity"),
        CheckConstraint("school_open = true OR student_attendance_required = false", name="ck_school_calendar_days_closed_no_student_attendance"),
    )
...
class SchoolCalendarEvent(BaseModel):
    ...
    event_type: Mapped[SchoolCalendarEventType] = mapped_column(
        SQLEnum(SchoolCalendarEventType, name="school_calendar_event_type", schema=PUBLIC_SCHEMA, values_callable=enum_values),
        nullable=False,
    )
    ...
    audience: Mapped[SchoolCalendarEventAudience] = mapped_column(
        SQLEnum(SchoolCalendarEventAudience, name="school_calendar_event_audience", schema=PUBLIC_SCHEMA, values_callable=enum_values),
        nullable=False,
    ...
    status: Mapped[SchoolCalendarEventStatus] = mapped_column(
        SQLEnum(SchoolCalendarEventStatus, name="school_calendar_event_status", schema=PUBLIC_SCHEMA, values_callable=enum_values),
        nullable=False,
    ...
    __table_args__ = (
        Index("ix_school_calendar_events_tenant_status_start", "tenant_id", "status", "starts_at"),
        CheckConstraint("ends_at > starts_at", name="ck_school_calendar_events_ends_after_start"),
        CheckConstraint("status <> 'published' OR published_at IS NOT NULL", name="ck_school_calendar_events_published_at"),
        CheckConstraint("status <> 'cancelled' OR cancelled_at IS NOT NULL", name="ck_school_calendar_events_cancelled_at"),
    )
...
class SchoolCalendarLifecycleAudit(BaseModel):
    ...
    acting_admin_id: Mapped[uuid.UUID | None] = mapped_column(UUID, ForeignKey("tenant_admins.id", ondelete="SET NULL"), nullable=True)
    reason: Mapped[str | None] = mapped_column(String(500), nullable=True)
    metadata_json: Mapped[dict | None] = mapped_column(JSONB, nullable=True)

    __table_args__ = (
        Index("ix_school_calendar_lifecycle_audits_tenant_entity", "tenant_id", "entity_type", "entity_id"),
