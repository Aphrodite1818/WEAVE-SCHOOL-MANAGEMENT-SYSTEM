...
class BulkImportParser:
    ...
    def _decode_csv_bytes(
        ...
        """Decode CSV bytes using common UTF encodings"""
    ...
    def resolve_file_type(
        *,
        ...
        """Resolve file type from an explicit input or filename extension"""
    ...
    def parse_csv_bytes(
        *,
        ...
        text = BulkImportParser._decode_csv_bytes(file_bytes)
        ...
        if not reader.fieldnames:
            raise ImportParserError("CSV file must contain a header row")
        

        headers = BulkImportParser._clean_headers(reader.fieldnames)

        if not headers:
            raise ImportParserError("CSV file must contain at least one valid column header .")
        
        BulkImportParser._ensure_unique_headers(headers)
        ...
        for row_number , row in enumerate(reader , start = 2):
            raw_data = {
                headers[index] : BulkImportParser._to_serializable_value(value)
                for index , value in enumerate(row.values())
                if index < len(headers)
            }


            if BulkImportParser._is_blank_row(raw_data):
                continue
    ...
    def parse_xlsx_bytes(
        *,
        ...
        """
        PARSE XLSX bytes into raw dictionaries
        """
        ...
        if worksheet is None:
            raise ImportParserError("Workbook has no active worksheets")
        ...
        headers = BulkImportParser._clean_headers(header_row)
    ...
    async def parse_upload(
        ...
        *,
        ...
        """Read an uploaded file and return parsed rows"""
        ...
        if len(file_bytes) > max_file_size_bytes:
            raise ImportParserError(
                f"Uploaded file is too large . Maximum allowed size is {max_file_size_bytes}"
            )
