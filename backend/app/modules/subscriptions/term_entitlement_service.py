...
class TermPlanEntitlementService:
    ...
    async def activate_free(
        db: AsyncSession, tenant_id: uuid.UUID, term_id: uuid.UUID, admin_id: uuid.UUID | None
    ) -> TermPlanEntitlement:
        term = await TermPlanEntitlementService._term(db, tenant_id, term_id, lock=True)
        if term.status == AcademicTermStatus.CLOSED:
            raise ConflictException("A closed academic term cannot receive a new plan entitlement.")
    ...
    async def initialize_paid_checkout(
        ...
        term = await TermPlanEntitlementService._term(db, tenant_id, term_id, lock=True)
        if term.status == AcademicTermStatus.CLOSED:
            raise ConflictException("A closed academic term cannot be upgraded.")
    ...
    async def activate_verified_transaction(
        ...
        *,
        ...
        existing = await TermPlanEntitlementService.get_active(
            db, transaction.tenant_id, transaction.academic_term_id, lock=True
        )
        if existing and existing.payment_transaction_id == transaction.id:
            return existing
        now = datetime.now(timezone.utc)
        if existing:
    ...
    async def close_for_term(
        db: AsyncSession, tenant_id: uuid.UUID, term_id: uuid.UUID, *, reason: str = "term_closed"
    ) -> None:
        entitlement = await TermPlanEntitlementService.get_active(db, tenant_id, term_id, lock=True)
        if entitlement:
