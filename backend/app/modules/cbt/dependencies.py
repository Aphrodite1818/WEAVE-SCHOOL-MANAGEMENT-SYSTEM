...
cbt_server_bearer = HTTPBearer(
    ...
    description = (
        "Machine credential issued to a paired Weave CBT server"
    ),
    auto_error = False
...
async def get_current_cbt_server(
    ...
    if authorization is None:
        raise UnauthorizedException(
            detail = "CBT server authentication is required"
        )
...
CurrentCBTServer = Annotated[
    AuthenticatedCBTServer,
    Security(get_current_cbt_server)
]