# ======================================#
# backend.app.modules.cbt.sync.models.py
# =====================================#
...
from app.modules.cbt.sync.enums import CBTSyncOperation, CBTSyncEntityType
...
class CBTSyncTenantState(TimestampMixin, Base):
    """
    Store the latest durable CBT synchronization cursor for one tenant

    The recorder will lock this row with SELECT... FOR UPDATE before
    incrementing the cursor and inserting a CBTSyncChange

    This prevent concurrent transactions for the same tenant from
    producing cursors that will become visible out of commit order
    """
    ...
    last_cursor: Mapped[int] = mapped_column(
        BigInteger, nullable=True, default=0, server_default=text("0")
    )


class CBTSyncChange(BaseModel):
    """
    Durable tenant-scoped log for CBT-visible mutations

    Rows are retained for incremental synchronization and offline recovery.
    They are not deleted simply because a live WebSocket delivery succeeds
    """
    ...
    __table_args__ = (
        # tenant_id and cursor
        # can have only 1 unique record
