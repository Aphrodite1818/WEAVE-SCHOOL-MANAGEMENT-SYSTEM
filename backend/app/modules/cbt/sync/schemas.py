# ======================================#
# backend.appcl.modules.cbt.sync.schemas
# ======================================#

from __future__ import annotations

import uuid
from datetime import datetime
from typing import Any

from pydantic import BaseModel, Field, model_validator

from app.modules.cbt.sync.enums import (
    CBTSyncEntityType,
    CBTSyncOperation,
)


class CBTSyncMutation(BaseModel):
    """
    Internal representation of one CBT-visible business mutation.

    This is what domain/projector code gives to the sync recorder.
    """

    entity_type: CBTSyncEntityType
    entity_id: uuid.UUID
    operation: CBTSyncOperation

    schema_version: int = Field(default=1, ge=1)

    payload: dict[str, Any] | None = None

    @model_validator(mode="after")
    def validate_payload(self) -> "CBTSyncMutation":
        if (
            self.operation
            in {
                CBTSyncOperation.CREATED,
                CBTSyncOperation.UPDATED,
            }
            and self.payload is None
        ):
            raise ValueError("Created and updated CBT sync mutations require a payload.")

        return self


class CBTSyncChangeResponse(BaseModel):
    """
    Stable synchronization event delivered to the local CBT server.

    The same schema is used for both live WebSocket delivery and
    HTTP delta recovery.
    """

    event_id: uuid.UUID

    cursor: int

    entity_type: CBTSyncEntityType
    entity_id: uuid.UUID
    operation: CBTSyncOperation

    schema_version: int

    payload: dict[str, Any] | None

    occurred_at: datetime


class CBTSyncDeltaResponse(BaseModel):
    """
    Ordered batch of synchronization changes returned during recovery.
    """

    from_cursor: int

    next_cursor: int

    has_more: bool

    changes: list[CBTSyncChangeResponse]




class CBTSyncNotification(BaseModel):
    """Internal PostgreSQL NOTIFY envelope."""

    change_id: uuid.UUID
    tenant_id: uuid.UUID
    cursor: int = Field(ge=1)