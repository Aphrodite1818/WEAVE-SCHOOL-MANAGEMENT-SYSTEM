...
from app.modules.teachers.models import TeacherAccount, TeacherAccountStatus, TeacherMembership, TeacherMembershipStatus
...
def build_bootstrap_sections(session: Session, *, tenant_id: uuid.UUID) -> dict[str, list[BaseModel]]:
    sections: dict[str, list[BaseModel]] = {}

    session_rows = _tenant_rows(session, AcademicSession, tenant_id)
    visible_sessions = {row.id: row for row in session_rows if row.is_current and row.status in {AcademicSessionStatus.OPEN, AcademicSessionStatus.CLOSING}}
    sections["sessions"] = _ordered([CBTAcademicSessionSnapshot(id=row.id, name=row.name, status=_value(row.status), is_current=row.is_current) for row in visible_sessions.values()])

    term_rows = _tenant_rows(session, AcademicTerm, tenant_id)
    visible_terms = {row.id: row for row in term_rows if row.academic_session_id in visible_sessions and row.is_current and row.status in {AcademicTermStatus.OPEN, AcademicTermStatus.CLOSING}}
    sections["terms"] = _ordered([CBTAcademicTermSnapshot(id=row.id, academic_session_id=row.academic_session_id, name=_value(row.name), status=_value(row.status), is_current=row.is_current) for row in visible_terms.values()])

    level_rows = _tenant_rows(session, AcademicLevel, tenant_id)
    visible_levels = {row.id: row for row in level_rows if _visible(row)}
    sections["levels"] = _ordered([CBTAcademicLevelSnapshot(id=row.id, name=row.name, category=_value(row.category), position=row.position) for row in visible_levels.values()])

    arm_rows = _tenant_rows(session, ArmLabel, tenant_id)
    visible_arms = {row.id: row for row in arm_rows if _visible(row)}
    sections["arm_labels"] = _ordered([CBTArmLabelSnapshot(id=row.id, label=row.label) for row in visible_arms.values()])

    canonical_department_rows = _tenant_rows(session, Department, tenant_id)
    visible_canonical_departments = {row.id: row for row in canonical_department_rows if _visible(row)}
    ...
    visible_classes = {
        ...
        if _visible(row) and row.academic_level_id in visible_levels and row.arm_label_id in visible_arms
    }
    ...
    for row in ctd_rows:
        ...
        if term is None or term.status != AcademicTermStatus.OPEN or classroom is None or link is None or link.academic_level_id != classroom.academic_level_id:
            continue
        visible_ctds[row.id] = row
        class_term_department[(row.class_id, row.academic_term_id)] = row.academic_level_department_id
    ...
    sections["subjects"] = _ordered([CBTSubjectSnapshot(id=row.id, name=row.name, code=row.code, is_active=row.is_active) for row in visible_subjects.values()])

    curriculum_rows = _tenant_rows(session, Curriculum, tenant_id)
    visible_curricula = {row.id: row for row in curriculum_rows if row.academic_level_id in visible_levels}
    sections["curricula"] = _ordered([CBTCurriculumSnapshot(id=row.id, academic_level_id=row.academic_level_id) for row in visible_curricula.values()])
    ...
    visible_curriculum_subjects = {
        ...
        if _visible(row) and row.curriculum_id in visible_curricula and row.subject_id in visible_subjects
    }
    ...
    for row in offering_rows:
        ...
        offering_scopes.add((row.curriculum_subject_id, row.academic_term_id, row.academic_level_department_id))
    ...
    visible_schemes = {row.id: row for row in scheme_rows if row.status == AssessmentSchemeStatus.ACTIVE}
    sections["assessment_schemes"] = _ordered([CBTAssessmentSchemeSnapshot(id=row.id, name=row.name, status=_value(row.status)) for row in visible_schemes.values()])
    ...
    sections["assessment_components"] = _ordered([
        ...
        for row in component_rows if row.is_active and row.assessment_scheme_id in visible_schemes
    ])
    ...
    sections["admins"] = _ordered([
        ...
        for row in admin_rows if row.account_status == TenantAdminStatus.ACTIVE and row.is_active and row.is_verified
    ])

    teacher_rows = list(session.execute(
    ...
    for membership, account in teacher_rows:
        if membership.status != TeacherMembershipStatus.ACTIVE or account.account_status != TeacherAccountStatus.ACTIVE or not account.is_active:
            continue
        visible_teachers[membership.id] = (membership, account)
        teacher_snapshots.append(CBTTeacherSnapshot(
            id=membership.id,
    ...
    enrollment_rows = list(session.execute(
    ...
    for enrollment, student in enrollment_rows:
        ...
        if (
            ...
            continue
        enrollment_snapshots.append(CBTStudentEnrollmentSnapshot(
            id=enrollment.id,
    ...
    if current_open_term is not None:
        ...
        for assignment in assignment_rows:
            ...
            if not (
                ...
                or (assignment.curriculum_subject_id, current_open_term.id, department_id) in offering_scopes
            ):
                continue
            assignment_snapshots.append(CBTTeacherAssignmentSnapshot(
                id=assignment.id,
