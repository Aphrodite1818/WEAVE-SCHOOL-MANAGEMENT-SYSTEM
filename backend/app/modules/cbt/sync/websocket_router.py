...
def _extract_bearer_credential(websocket: WebSocket) -> str | None:
    ...
    if not separator or scheme.lower() != "bearer" or not credential.strip(): return None
    return credential.strip()


async def _authenticate_server(credential: str) -> AuthenticatedCBTServer:
    async with AsyncSessionLocal() as db:
        return await CBTMachineAuthService.authenticate_server(db, server_credential=credential)
...
async def cbt_sync_stream(websocket: WebSocket) -> None:
    ...
    if credential is None:
        await websocket.close(code=AUTHENTICATION_CLOSE_CODE, reason="CBT server authentication is required.")
        return
    try:
        try:
            current_server = await _authenticate_server(credential)
        except ForbiddenException:
            await websocket.close(code=FORBIDDEN_CLOSE_CODE, reason="CBT server is not permitted to synchronize.")
            return
        except AppException:
            await websocket.close(code=AUTHENTICATION_CLOSE_CODE, reason="CBT server authentication failed.")
            return

        await websocket.accept()
        connection = await cbt_connection_manager.register(server_id=current_server.server_id, tenant_id=current_server.tenant_id, websocket=websocket)
        await _send(connection, {
        ...
        while True:
            ...
            if not isinstance(message, dict):
                await _send(connection, {"type": "error", "code": "invalid_control_frame", "message": "Control frames must be JSON objects."})
                continue
            if message.get("type") == "ping":
                await _send(connection, {"type": "pong"})
                continue
            await _send(connection, {"type": "error", "code": "unsupported_control_frame", "message": "Unsupported CBT synchronization control frame."})
        ...
        if connection is not None:
            await cbt_connection_manager.unregister(server_id=connection.server_id, websocket=websocket)
