...
class MetricsRepository:
    ...
    async def tenant_growth(db: AsyncSession) -> list[PeriodCount]:
        ...
        rows = (
            await db.execute(
                select(
                    growth_period.label("period"), func.count(Tenant.id).label("value")
                )
    ...
    async def teacher_class_sizes(
        ...
        *,
        ...
        rows = (
            await db.execute(
                select(
                    ClassRoom.id, ClassRoom.name, ClassRoom.arm, func.count(Student.id)
                )
                .select_from(ClassRoom)
                .outerjoin(
                    Student,
            ...
            (
                await db.execute(
                    select(Announcement.id).where(
                        ...
                        Announcement.created_by_actor_type
    ...
    async def notification_summary_for_actor(
        ...
        *,
        ...
        return (
            ...
            read_count,
            [
                LabelCount(label=label, value=value)
                for label, value in by_source.items()
            ],
        )
    ...
    async def submitted_results_for_student(
        ...
        *,
        ...
        rows = (
            await db.execute(
                select(
                    StudentSubjectResult.grade, StudentSubjectResult.total_score
                ).where(
                    StudentSubjectResult.tenant_id == tenant_id,
