"""OTP verification, password-reset tokens, and legacy invite compatibility."""
...
from app.core.utils.email_templates import get_otp_email_html, get_user_invite_email_html
...
from app.modules.auth.schemas import TenantActivationRequest, UserInviteAcceptanceRequest, VerifyOTP
...
class UserInviteService:
    """Compatibility layer for remaining generic invite callers."""
    ...
    async def create_invite_record(
        ...
        *,
        ...
        return (
            ...
            f"/invite?token={quote(raw_token, safe='')}"
        )
    ...
    async def send_invite_email(
        *,
        ...
        html = get_user_invite_email_html(user_name, school_name, invite_link)
        if background_tasks is not None:
            background_tasks.add_task(
                send_email,
                email,
            ...
            await send_email(email, f"Join {school_name} on Weave", html, True)

    @staticmethod
    async def get_invite_status(db: AsyncSession, token: str) -> dict[str, str | None]:
        records = (
            await db.execute(
                select(AuthRecord)
                .where(
                    ...
                    AuthRecord.is_used.is_(False),
                    AuthRecord.expires_at > _utc_now(),
                )
                .order_by(AuthRecord.created_at.desc())
            )
        ).scalars().all()
        record = next(
            (item for item in records if verify_auth_secret(token, item.hashed_value)),
            None,
        )
        if record is None:
    ...
    async def accept_invite(
        ...
        if record is None:
            raise BadRequestException("Invite is invalid or expired.")
        ...
        db.add(record)
        await db.commit()
        return {"detail": "Account setup completed successfully."}
