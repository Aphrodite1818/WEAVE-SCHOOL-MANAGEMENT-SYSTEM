#======================================#
#             auth/models.py           #
#======================================#

from datetime import datetime, timezone
from enum import Enum as PyEnum
import uuid
from sqlalchemy import Boolean, DateTime, Enum as SQLEnum, ForeignKey, String , UUID, Text , CheckConstraint , Index
from sqlalchemy.orm import Mapped, mapped_column

from app.shared.base_model import BaseModel, PUBLIC_SCHEMA , Base
from app.shared.mixins import TimestampMixin , UUIDMixin 


class AuthPurpose(str, PyEnum):
    """Enumeration of supported auth values."""
    VERIFICATION = "verification"
    PASSWORD_RESET = "password_reset"
    TENANT_ACTIVATION = "tenant_activation"
    USER_INVITE = "user_invite"


class AuthRecord(BaseModel):
    """Represent the AuthRecord type."""
    __tablename__ = "auth"

    email: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    hashed_value: Mapped[str] = mapped_column(String(255), nullable=False)
    purpose: Mapped[AuthPurpose] = mapped_column(
        SQLEnum(
            AuthPurpose,
            name="otppurpose",
            schema=PUBLIC_SCHEMA,
            values_callable=lambda enum_cls: [item.value for item in enum_cls],
        ),
        nullable=False,
    )
    expires_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    is_used: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)




class AuthSessionActorType(str , PyEnum):#didnt reuse AuthIdentity.ActorType because it is tenant-scoped and does not include superadmin
    """
    Actors that can own a login session
    """

    SUPERADMIN = "superadmin"
    TENANT_ADMIN = "tenant_admin"
    TEACHER = "teacher"
    STAFF = "staff"
    PARENT = "parent"
    STUDENT = "student"

class AuthSession(Base, TimestampMixin , UUIDMixin):
    """
    Represents one persistent login session

    e.g
    - User logs in on Chrome desktop : One AuthSession


    Access tokens remain short-lived and stateless
    Refresh tokens are stored separately  
    """
    __tablename__ = "auth_sessions"


    tenant_id : Mapped[uuid.UUID | None ] = mapped_column(
        UUID(as_uuid = True),
        ForeignKey(f"tenants.id", ondelete="CASCADE"),
        nullable=True,
        index=True,
        doc = "Null for superadmin sessions; required for tenant actor sessions"
    )



    actor_type : Mapped[AuthSessionActorType] = mapped_column(
        SQLEnum(
            AuthSessionActorType,
            name = "auth_session_actor_type",
            schema = PUBLIC_SCHEMA,
            values_callable = lambda enum_cls : [item.value for item in enum_cls]
        ),
        nullable = False ,
        index = True
    )

    actor_id : Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        nullable = False ,
        index = True,
        doc = "ID of the actor in its own table . Polymorphic , so not fk here"
    )


    #jti = jwt token id unique id for a specific token
    session_jti : Mapped[str] = mapped_column(
        String(64),
        nullable = False,
        unique = True,
        index = True,
        doc = "Stable unique Id for this session/token "
    )


    user_agent : Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
        doc = "stores what browser was used during the session"
    )



    ip_address : Mapped[str | None] = mapped_column(
        String(45),
        nullable = True,
        doc = "IPv4 or IPv6 address captured at login"
    )


    remember_me : Mapped[bool] = mapped_column(
        Boolean , 
        default = False,
        server_default="false",
        nullable = False
    )



    last_used_at : Mapped[datetime | None] = mapped_column(
        DateTime(timezone =True ),
        nullable = True
    )


    expires_at : Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable = False,
        index = True
    )





    revoked_at : Mapped[datetime | None] = mapped_column(
        DateTime(timezone = True),
        nullable = True ,
        index = True
    )


    revoked_reason : Mapped[str | None] = mapped_column(
        String(50),
        nullable = True,
        doc = "Examples : logout, expired , admin_revoked, refresh_reuse_detected"
    )


    compromised_at : Mapped[datetime | None] = mapped_column(
        DateTime(timezone= True),
        nullable =True ,
        doc = "Set when refresh-token reuse is detected"
    )



    __table_args__ = (
        CheckConstraint(
            """
            (
                actor_type = 'superadmin'
                AND tenant_id IS NULL
            )
            OR
            (
                actor_type <> 'superadmin'
                AND tenant_id IS NOT NULL
            )
            """,
            name="ck_auth_sessions_tenant_scope",
        ),
        Index(
            "ix_auth_sessions_actor",
            "actor_type",
            "actor_id",
        ),
        Index(
            "ix_auth_sessions_tenant_actor",
            "tenant_id",
            "actor_type",
            "actor_id",
        ),
        Index(
            "ix_auth_sessions_active_lookup",
            "actor_type",
            "actor_id",
            "revoked_at",
            "expires_at",
        ),
    )

    @property
    def is_revoked(self) -> bool:
        """Return whether the session has been explicitly revoked."""

        return self.revoked_at is not None

    @property
    def is_compromised(self) -> bool:
        """Return whether refresh-token reuse has marked the session unsafe."""

        return self.compromised_at is not None

    @property
    def is_expired(self) -> bool:
        """Return whether the session has passed its expiry time."""

        return datetime.now(timezone.utc) >= self.expires_at

    @property
    def is_active(self) -> bool:
        """Return whether the session can still be used."""

        return not self.is_revoked and not self.is_compromised and not self.is_expired
    







class AuthRefreshToken(UUIDMixin , TimestampMixin , Base):
    """
    Stores hashed refresh tokens for refresh-token rotation

    the raw refresh token is never saved 


    flow 
    -login created session + first refresh token
    -refresh marks old token as used
    -refresh creates a new refresh token
    -reuse of an old token marks the session as compromised
    """

    __tablename__ = "auth_refresh_tokens"


    session_id : Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid = True),
        ForeignKey(f"auth_sessions.id", ondelete = "CASCADE"),
        nullable = False,
        index = True
    )



    token_hash : Mapped[str] = mapped_column(
        String(255),
        nullable = False,
        unique = True ,
        index = True ,
        
    )


    token_jti : Mapped[str] = mapped_column(
        String(64),
        nullable = False,
        unique=True ,
        index=True,
        doc = "Unique ID for this specific refresh token"
    )


    issued_ip_address : Mapped[str | None] = mapped_column(
        String(45),
        nullable = True
    ) 


    issued_user_agent : Mapped[str | None] = mapped_column(
        Text ,
        nullable = True
    )

    expires_at : Mapped[datetime] = mapped_column(
        DateTime(timezone = True),
        nullable = False,
        index = True
    )


    expires_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        index=True,
    )

    used_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        doc="Set when this refresh token is successfully rotated.",
    )

    revoked_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        index=True,
    )

    revoked_reason: Mapped[str | None] = mapped_column(
        String(100),
        nullable=True,
    )

    replaced_by_token_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey(
            f"auth_refresh_tokens.id",
            ondelete="SET NULL",
        ),
        nullable=True,
    )

    reuse_detected_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        doc="Set when this token is presented again after use/revocation.",
    )

    __table_args__ = (
        Index(
            "ix_auth_refresh_tokens_session_active",
            "session_id",
            "revoked_at",
            "expires_at",
        ),
        Index(
            "ix_auth_refresh_tokens_rotation_state",
            "session_id",
            "used_at",
            "revoked_at",
        ),
    )

    @property
    def is_used(self) -> bool:
        """Return whether this refresh token has already been rotated."""

        return self.used_at is not None

    @property
    def is_revoked(self) -> bool:
        """Return whether this refresh token has been explicitly revoked."""

        return self.revoked_at is not None

    @property
    def is_expired(self) -> bool:
        """Return whether this refresh token has passed its expiry time."""

        return datetime.now(timezone.utc) >= self.expires_at

    @property
    def is_reuse_detected(self) -> bool:
        """Return whether this token has already triggered reuse detection."""

        return self.reuse_detected_at is not None

    @property
    def is_active(self) -> bool:
        """Return whether this refresh token can still be exchanged."""

        return not self.is_used and not self.is_revoked and not self.is_expired