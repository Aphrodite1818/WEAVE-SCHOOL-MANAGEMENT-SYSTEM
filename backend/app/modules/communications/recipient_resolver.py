...
from app.modules.communications.schemas import ActorIdentity, AnnouncementAudienceCreate, RecipientTarget
...
from app.modules.students.models import AcademicStatus, Student, StudentAccountStatus, StudentParentLink, StudentParentLinkStatus
...
class RecipientResolver:
    ...
    async def available_direct_recipients(db: AsyncSession, sender) -> list[ResolvedRecipient]:
        ...
        if isinstance(sender, Teacher):
            tenant_id = sender.tenant_id
            recipients = []
            recipients.extend(await RecipientResolver._students_taught_by_teacher(db, sender, group="Students I teach"))
            recipients.extend(await RecipientResolver._parents_for_teacher_students(db, sender, group="Parents of students I teach"))
            recipients.extend(await RecipientResolver._teachers(db, tenant_id, group="Teachers", exclude_id=sender.id))
            recipients.extend(await RecipientResolver._tenant_admins(db, tenant_id=tenant_id, group="School administrators"))
            return _dedupe(recipients)
        if isinstance(sender, Student):
            if sender.class_id is None:
                return await RecipientResolver._tenant_admins(db, tenant_id=sender.tenant_id, group="School administration")
            recipients = []
            recipients.extend(await RecipientResolver._class_teacher(db, sender.tenant_id, sender.class_id, group="My class teacher"))
            recipients.extend(await RecipientResolver._subject_teachers_for_class(db, sender.tenant_id, sender.class_id, group="My subject teachers"))
            recipients.extend(await RecipientResolver._tenant_admins(db, tenant_id=sender.tenant_id, group="School administration"))
            return _dedupe(recipients)
        if isinstance(sender, Parent):
            recipients = []
            class_ids = await RecipientResolver._parent_child_class_ids(db, sender)
            for class_id in class_ids:
                recipients.extend(await RecipientResolver._class_teacher(db, sender.tenant_id, class_id, group="My child's class teacher"))
                recipients.extend(await RecipientResolver._subject_teachers_for_class(db, sender.tenant_id, class_id, group="My child's subject teachers"))
            recipients.extend(await RecipientResolver._teachers(db, sender.tenant_id, group="Other teachers"))
            recipients.extend(await RecipientResolver._tenant_admins(db, tenant_id=sender.tenant_id, group="School administration"))
            return _dedupe(recipients)
        raise ForbiddenException("Unsupported communication actor")

    @staticmethod
    async def resolve_direct_target(db: AsyncSession, sender, target: RecipientTarget) -> ResolvedRecipient:
        actor_type = target.actor_type if isinstance(target.actor_type, CommunicationActorType) else CommunicationActorType(target.actor_type)
    ...
    async def resolve_announcement_audience(
        ...
        *,
        ...
        if isinstance(sender, SuperAdmin):
            ...
            for audience in audiences:
                ...
                if audience_type not in allowed:
                    raise ForbiddenException("Superadmin announcements can target tenant admins only")
                recipients.extend(await RecipientResolver._resolve_superadmin_audience(db, audience))
            ...
            for audience in audiences:
                ...
                resolved, audience_excluded = await RecipientResolver._resolve_tenant_admin_audience(db, sender.tenant_id, audience)
        ...
        if not recipients:
            raise BadRequestException("No active recipients match this audience. Choose a different audience before saving or publishing.")
        return recipients, excluded

    @staticmethod
    def _audience_type(value: AnnouncementAudienceType | str) -> AnnouncementAudienceType:
        return value if isinstance(value, AnnouncementAudienceType) else AnnouncementAudienceType(value)

    @staticmethod
    async def _superadmins(db: AsyncSession, *, group: str) -> list[ResolvedRecipient]:
        rows = (await db.execute(select(SuperAdmin).where(SuperAdmin.is_active.is_(True)))).scalars().all()
        return [ResolvedRecipient(CommunicationActorType.SUPERADMIN, row.id, None, row.email, group) for row in rows]

    @staticmethod
    async def _tenant_admins(db: AsyncSession, *, tenant_id: uuid.UUID | None = None, group: str = "Tenant admins") -> list[ResolvedRecipient]:
        ...
        return [ResolvedRecipient(CommunicationActorType.TENANT_ADMIN, row.id, row.tenant_id, row.email, group) for row in rows]

    @staticmethod
    async def _teachers(db: AsyncSession, tenant_id: uuid.UUID, *, group: str, exclude_id: uuid.UUID | None = None) -> list[ResolvedRecipient]:
        stmt = (
            select(
                Teacher.id,
                Teacher.staff_id,
    ...
    @staticmethod
    async def _students(db: AsyncSession, tenant_id: uuid.UUID, *, group: str) -> list[ResolvedRecipient]:
        rows = (
            await db.execute(
    ...
    @staticmethod
    async def _parents(db: AsyncSession, tenant_id: uuid.UUID, *, group: str) -> list[ResolvedRecipient]:
        rows = (
            await db.execute(
                select(
                    Parent.id,
                ...
                .where(
                    ...
                    Parent.status.in_([ParentMembershipStatus.ACTIVE, ParentMembershipStatus.READ_ONLY]),
    ...
    @staticmethod
    async def _students_taught_by_teacher(db: AsyncSession, teacher: Teacher, *, group: str) -> list[ResolvedRecipient]:
    ...
    @staticmethod
    async def _parents_for_teacher_students(db: AsyncSession, teacher: Teacher, *, group: str) -> list[ResolvedRecipient]:
        class_ids = await RecipientResolver._assigned_class_ids_for_teacher(db, teacher)
        if not class_ids:
            return []
        rows = (
            await db.execute(
                ...
                .where(
                    ...
                    Student.class_id.in_(class_ids),
                    StudentParentLink.status.in_([StudentParentLinkStatus.ACTIVE, StudentParentLinkStatus.READ_ONLY]),
                    Parent.status.in_([ParentMembershipStatus.ACTIVE, ParentMembershipStatus.READ_ONLY]),
                    ParentAccount.is_active.is_(True),
                    ParentAccount.is_verified.is_(True),
                    ParentAccount.account_status == ParentAccountStatus.ACTIVE,
    ...
    @staticmethod
    async def _class_teacher(db: AsyncSession, tenant_id: uuid.UUID, class_id: uuid.UUID, *, group: str) -> list[ResolvedRecipient]:
        row = (
            await db.execute(
                select(
                    Teacher.id,
    ...
    @staticmethod
    async def _subject_teachers_for_class(db: AsyncSession, tenant_id: uuid.UUID, class_id: uuid.UUID, *, group: str) -> list[ResolvedRecipient]:
        rows = (
            await db.execute(
                select(
                    Teacher.id,
    ...
    @staticmethod
    async def _resolve_superadmin_audience(db: AsyncSession, audience: AnnouncementAudienceCreate) -> list[ResolvedRecipient]:
        audience_type = RecipientResolver._audience_type(audience.audience_type)
        ...
        if audience_type == AnnouncementAudienceType.SELECTED_TENANT_ADMINS:
            ...
            return [item for item in await RecipientResolver._tenant_admins(db) if item.actor_id == audience.actor_id]
    ...
    async def _resolve_tenant_admin_audience(
        ...
        if audience_type == AnnouncementAudienceType.SELECTED_TEACHERS:
            ...
            return [item for item in await RecipientResolver._teachers(db, tenant_id, group="Selected teachers") if item.actor_id == audience.actor_id], []
        ...
        if audience_type == AnnouncementAudienceType.SELECTED_STUDENTS:
            ...
            return [item for item in await RecipientResolver._students(db, tenant_id, group="Selected students") if item.actor_id == audience.actor_id], []
        ...
        if audience_type == AnnouncementAudienceType.SELECTED_PARENTS:
            ...
            return [item for item in await RecipientResolver._parents(db, tenant_id, group="Selected parents") if item.actor_id == audience.actor_id], []
        if audience_type == AnnouncementAudienceType.CLASS_STUDENTS:
            ...
            return [item for item in rows if await RecipientResolver._student_in_class(db, item.actor_id, audience.class_id)], []
        if audience_type == AnnouncementAudienceType.CLASS_PARENTS:
            if audience.class_id is None:
                raise BadRequestException("Choose a class")
            recipients = await RecipientResolver._parents_for_class(db, tenant_id, audience.class_id)
            missing = await RecipientResolver._students_without_parents_count(db, tenant_id, audience.class_id)
            excluded = [f"{missing} student(s) without a linked parent"] if missing else []
            return recipients, excluded
        raise ForbiddenException("Unsupported school audience")

    @staticmethod
    async def _student_in_class(db: AsyncSession, student_id: uuid.UUID, class_id: uuid.UUID) -> bool:
        return bool((await db.execute(select(func.count()).select_from(Student).where(Student.id == student_id, Student.class_id == class_id))).scalar_one())

    @staticmethod
    async def _parents_for_class(db: AsyncSession, tenant_id: uuid.UUID, class_id: uuid.UUID) -> list[ResolvedRecipient]:
        rows = (
            await db.execute(
                select(
                    Parent.id,
    ...
    @staticmethod
    async def _students_without_parents_count(db: AsyncSession, tenant_id: uuid.UUID, class_id: uuid.UUID) -> int:
        active_parent_link = (
