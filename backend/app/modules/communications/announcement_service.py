...
from app.modules.communications.enums import AnnouncementStatus, CommunicationActorType, NotificationSourceType
...
from app.modules.communications.recipient_resolver import RecipientResolver, actor_tenant_id, actor_type_for
...
class AnnouncementService:
    ...
    async def _resolve_audience_or_raise(db: AsyncSession, *, actor, audiences):
        normalized_audiences = [
            AnnouncementAudienceCreate.model_validate(audience) if isinstance(audience, dict) else audience
            for audience in audiences
        ]
        return await RecipientResolver.resolve_announcement_audience(db, sender=actor, audiences=normalized_audiences)

    @staticmethod
    async def create(db: AsyncSession, *, actor, payload) -> Announcement:
        AnnouncementService._ensure_creator(actor)
        await AnnouncementService._resolve_audience_or_raise(db, actor=actor, audiences=payload.audiences)
    ...
    @staticmethod
    async def list_manageable(db: AsyncSession, *, actor, status: AnnouncementStatus | None, offset: int, limit: int):
        AnnouncementService._ensure_creator(actor)
        stmt = select(Announcement).options(selectinload(Announcement.audiences)).where(
        ...
        rows = (await db.execute(stmt.order_by(Announcement.updated_at.desc()).offset(offset).limit(limit))).unique().scalars().all()
        return list(rows), int(total)

    @staticmethod
    async def get_manageable(db: AsyncSession, *, actor, announcement_id: uuid.UUID) -> Announcement:
        ...
        if announcement.created_by_actor_type != actor_type_for(actor) or announcement.created_by_actor_id != actor.id:
            raise NotFoundException("Announcement not found")
    ...
    @staticmethod
    async def update(db: AsyncSession, *, actor, announcement_id: uuid.UUID, payload) -> Announcement:
        announcement = await AnnouncementService.get_manageable(db, actor=actor, announcement_id=announcement_id)
    ...
    async def preview(db: AsyncSession, *, actor, audiences):
        recipients, excluded = await AnnouncementService._resolve_audience_or_raise(db, actor=actor, audiences=audiences)
        label = recipients[0].group_label or "Audience" if recipients else "Audience"
        return label, recipients, excluded

    @staticmethod
    async def publish(db: AsyncSession, *, actor, announcement_id: uuid.UUID, publish_at=None) -> Announcement:
        announcement = await AnnouncementService.get_manageable(db, actor=actor, announcement_id=announcement_id)
        ...
        recipients, _ = await AnnouncementService._resolve_audience_or_raise(db, actor=actor, audiences=announcement.audiences)
