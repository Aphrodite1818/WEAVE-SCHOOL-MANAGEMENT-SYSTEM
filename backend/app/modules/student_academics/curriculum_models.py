...
class Curriculum(BaseModel):
    ...
    academic_level_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("academic_levels.id", ondelete="CASCADE"), nullable=False)
    __table_args__ = (UniqueConstraint("tenant_id", "academic_level_id", name="uq_curricula_tenant_level"),)
...
class CurriculumSubject(BaseModel):
    ...
    is_elective: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="false")
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default="true")
    __table_args__ = (
        UniqueConstraint("tenant_id", "curriculum_id", "subject_id", name="uq_curriculum_subject_tenant_curriculum_subject"),
...
class ClassTermDepartmentAssignment(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "class_id", "academic_term_id", name="uq_class_term_department_assignment"),
...
class CurriculumOffering(BaseModel):
    ...
    curriculum_subject_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("curriculum_subjects.id", ondelete="CASCADE"), nullable=False)
    academic_term_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("academic_terms.id", ondelete="CASCADE"), nullable=False)
