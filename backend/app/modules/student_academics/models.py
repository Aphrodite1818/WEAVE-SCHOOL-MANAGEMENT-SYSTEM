...
class AcademicSession(BaseModel):
    ...
    status: Mapped[AcademicSessionStatus] = mapped_column(
        SQLEnum(AcademicSessionStatus, name="academic_session_status", schema=PUBLIC_SCHEMA, values_callable=enum_values),
        nullable=False,
    ...
    __table_args__ = (
        ...
        CheckConstraint("next_academic_session_id IS NULL OR next_academic_session_id <> id", name="ck_academic_session_next_not_self"),
        CheckConstraint(
            """
        ...
        CheckConstraint("status <> 'closed' OR is_current = false", name="ck_closed_academic_session_not_current"),
    )
...
class TeacherAssignment(BaseModel):
    ...
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, server_default="true", nullable=False)
    effective_from: Mapped[date] = mapped_column(Date, nullable=False, server_default=text("CURRENT_DATE"))
...
class StudentProgressionRun(BaseModel):
    ...
    academic_session_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("academic_sessions.id", ondelete="RESTRICT"), nullable=False)
    next_academic_session_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("academic_sessions.id", ondelete="RESTRICT"), nullable=False)
    ...
    status: Mapped[StudentProgressionRunStatus] = mapped_column(
        SQLEnum(StudentProgressionRunStatus, name="student_progression_run_status", schema=PUBLIC_SCHEMA, values_callable=enum_values),
        nullable=False,
    ...
    initiated_by_admin_id: Mapped[uuid.UUID | None] = mapped_column(UUID, ForeignKey("tenant_admins.id", ondelete="SET NULL"), nullable=True)
    failure_reason: Mapped[str | None] = mapped_column(String(1000), nullable=True)

    __table_args__ = (
        UniqueConstraint("tenant_id", "idempotency_key", name="uq_student_progression_run_tenant_idempotency"),
        UniqueConstraint("tenant_id", "academic_session_id", name="uq_student_progression_run_tenant_session"),
...
class StudentProgressionItem(BaseModel):
    ...
    progression_run_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("student_progression_runs.id", ondelete="RESTRICT"), nullable=False)
    student_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("students.id", ondelete="RESTRICT"), nullable=False)
    from_enrollment_id: Mapped[uuid.UUID | None] = mapped_column(UUID, ForeignKey("student_enrollments.id", ondelete="RESTRICT"), nullable=True)
    to_enrollment_id: Mapped[uuid.UUID | None] = mapped_column(UUID, ForeignKey("student_enrollments.id", ondelete="RESTRICT"), nullable=True)
    from_class_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("classes.id", ondelete="RESTRICT"), nullable=False)
    to_class_id: Mapped[uuid.UUID | None] = mapped_column(UUID, ForeignKey("classes.id", ondelete="RESTRICT"), nullable=True)
    action: Mapped[StudentProgressionItemAction] = mapped_column(
        SQLEnum(StudentProgressionItemAction, name="student_progression_item_action", schema=PUBLIC_SCHEMA, values_callable=enum_values),
        nullable=False,
    )
    status: Mapped[StudentProgressionItemStatus] = mapped_column(
        SQLEnum(StudentProgressionItemStatus, name="student_progression_item_status", schema=PUBLIC_SCHEMA, values_callable=enum_values),
        nullable=False,
    )
    ...
    __table_args__ = (
        UniqueConstraint("progression_run_id", "student_id", name="uq_progression_item_run_student"),
        ...
        CheckConstraint("action <> 'promote' OR to_class_id IS NOT NULL", name="ck_progression_item_promotion_has_target"),
        CheckConstraint("action <> 'graduate' OR to_class_id IS NULL", name="ck_progression_item_graduation_no_target"),
    )
...
class ClassSubjectTeacher(BaseModel):
    ...
    is_core: Mapped[bool] = mapped_column(Boolean, default=True, server_default="true", nullable=False)
    sort_order: Mapped[int] = mapped_column(Integer, default=0, server_default="0", nullable=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, server_default="true", nullable=False)

    __table_args__ = (
        UniqueConstraint("tenant_id", "class_id", "subject_id", name="uq_class_subject_teacher_tenant_class_subject"),
        Index("ix_class_subject_teachers_tenant_membership_active", "tenant_id", "teacher_membership_id", "is_active"),
    )
...
class StudentSubjectResult(BaseModel):
    ...
    student_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("students.id", ondelete="RESTRICT"), nullable=False, index=True)
    class_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("classes.id", ondelete="RESTRICT"), nullable=False, index=True)
    subject_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("subjects.id", ondelete="RESTRICT"), nullable=False, index=True)
    teacher_membership_id: Mapped[uuid.UUID] = mapped_column(
        UUID,
        ForeignKey("teacher_memberships.id", ondelete="RESTRICT"),
        nullable=False,
    ...
    status: Mapped[AcademicResultStatus] = mapped_column(
        SQLEnum(AcademicResultStatus, name="academic_result_status", schema=PUBLIC_SCHEMA, values_callable=enum_values),
        nullable=False,
    ...
    __table_args__ = (
        ...
        Index("ix_student_subject_results_tenant_teacher_membership", "tenant_id", "teacher_membership_id"),
