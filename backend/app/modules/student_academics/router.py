...
tenant_admin_router = APIRouter(prefix="/tenant-admin/academic", tags=["Tenant Admin Academic"])
teacher_router = APIRouter(prefix="/teachers/me/academic", tags=["Teacher Academic"])
student_router = APIRouter(prefix="/students/me/academic", tags=["Student Academic"])
parent_router = APIRouter(prefix="/parents/me/children/{student_id}/academic", tags=["Parent Academic"])
...
async def _ensure_teacher_can_write_result(db: DbSession, teacher: Teacher, payload: StudentSubjectResultUpsert) -> None:
    if payload.teacher_assignment_id is None:
        return
    ...
    if existing is not None and _is_submitted_status(existing.status):
        raise ForbiddenException("Submitted scores are locked. Ask an admin to reopen or correct this result.")
...
async def list_sessions(db: DbSession, current_admin: CurrentTenantAdmin, skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=100)) -> AcademicSessionListResponse:
    items, total = await StudentAcademicService.list_academic_sessions(db, current_admin.tenant_id, skip=skip, limit=limit)
...
async def update_session(session_id: UUID, payload: AcademicSessionUpdate, db: DbSession, current_admin: CurrentTenantAdmin) -> AcademicSessionResponse:
    return await StudentAcademicService.update_academic_session(db, current_admin.tenant_id, session_id, payload)
...
async def list_terms(db: DbSession, current_admin: CurrentTenantAdmin, academic_session_id: UUID | None = Query(default=None), skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=100)) -> AcademicTermListResponse:
    items, total = await StudentAcademicService.list_academic_terms(db, current_admin.tenant_id, skip=skip, limit=limit, academic_session_id=academic_session_id)
...
async def update_term(term_id: UUID, payload: AcademicTermUpdate, db: DbSession, current_admin: CurrentTenantAdmin) -> AcademicTermResponse:
    return await StudentAcademicService.update_academic_term(db, current_admin.tenant_id, term_id, payload)
...
async def list_grading_scales(db: DbSession, current_admin: CurrentTenantAdmin, active_only: bool = Query(default=False), skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=100)) -> GradingScaleListResponse:
    items, total = await StudentAcademicService.list_grading_scales(db, current_admin.tenant_id, skip=skip, limit=limit, active_only=active_only)
...
async def update_grading_scale(scale_id: UUID, payload: GradingScaleUpdate, db: DbSession, current_admin: CurrentTenantAdmin) -> GradingScaleResponse:
    return await StudentAcademicService.update_grading_scale(db, current_admin.tenant_id, scale_id, payload)
...
async def list_subject_assignments(db: DbSession, current_admin: CurrentTenantAdmin, response: Response, class_id: UUID | None = Query(default=None), subject_id: UUID | None = Query(default=None), teacher_id: UUID | None = Query(default=None), active_only: bool = Query(default=False), skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=100)) -> ClassSubjectTeacherListResponse:
    response.headers["X-Deprecated-Endpoint"] = "subject-assignments"
    response.headers["Deprecation"] = "true"
    items, total = await StudentAcademicService.list_class_subject_teachers(db, current_admin.tenant_id, skip=skip, limit=limit, class_id=class_id, subject_id=subject_id, teacher_id=teacher_id, active_only=active_only)
    return ClassSubjectTeacherListResponse(items=items, total=total)


@tenant_admin_router.patch("/subject-assignments/{assignment_id}", response_model=ClassSubjectTeacherResponse)
async def update_subject_assignment(assignment_id: UUID, payload: ClassSubjectTeacherUpdate, db: DbSession, current_admin: CurrentTenantAdmin, response: Response) -> ClassSubjectTeacherResponse:
    response.headers["X-Deprecated-Endpoint"] = "subject-assignments"
    response.headers["Deprecation"] = "true"
    return await StudentAcademicService.update_class_subject_teacher(db, current_admin.tenant_id, assignment_id, payload)
...
async def list_teacher_assignments_admin(db: DbSession, current_admin: CurrentTenantAdmin, class_id: UUID | None = Query(default=None), teacher_id: UUID | None = Query(default=None), active_only: bool = Query(default=False), skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=100)) -> TeacherAssignmentListResponse:
    items, total = await StudentAcademicService.list_teacher_assignment_responses(db, current_admin.tenant_id, class_id=class_id, teacher_id=teacher_id, active_only=active_only, skip=skip, limit=limit)
    return TeacherAssignmentListResponse(items=items, total=total)


@tenant_admin_router.patch("/teacher-assignments/{assignment_id}/deactivate", response_model=TeacherAssignmentResponse)
async def deactivate_teacher_assignment(assignment_id: UUID, db: DbSession, current_admin: CurrentTenantAdmin) -> TeacherAssignmentResponse:
    return await StudentAcademicService.deactivate_teacher_assignment(db, current_admin.tenant_id, assignment_id)
...
async def upsert_admin_result(payload: StudentSubjectResultUpsert, db: DbSession, current_admin: CurrentTenantAdmin) -> StudentSubjectResultResponse:
...
async def list_admin_results(db: DbSession, current_admin: CurrentTenantAdmin, student_id: UUID | None = Query(default=None), class_id: UUID | None = Query(default=None), academic_session_id: UUID | None = Query(default=None), academic_term_id: UUID | None = Query(default=None), skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=100)) -> StudentSubjectResultListResponse:
    items, total = await StudentAcademicService.list_results(db, current_admin, skip=skip, limit=limit, student_id=student_id, class_id=class_id, academic_session_id=academic_session_id, academic_term_id=academic_term_id)
...
async def update_result_status(result_id: UUID, payload: StudentSubjectResultStatusUpdate, db: DbSession, current_admin: CurrentTenantAdmin) -> StudentSubjectResultResponse:
...
async def list_teacher_sessions(db: DbSession, current_teacher: CurrentTeacher, skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=100)) -> AcademicSessionListResponse:
    items, total = await StudentAcademicService.list_academic_sessions(db, current_teacher.tenant_id, skip=skip, limit=limit)
    return AcademicSessionListResponse(items=items, total=total)


@teacher_router.get("/terms", response_model=AcademicTermListResponse)
async def list_teacher_terms(db: DbSession, current_teacher: CurrentTeacher, academic_session_id: UUID | None = Query(default=None), skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=100)) -> AcademicTermListResponse:
    items, total = await StudentAcademicService.list_academic_terms(db, current_teacher.tenant_id, skip=skip, limit=limit, academic_session_id=academic_session_id)
    return AcademicTermListResponse(items=items, total=total)


@teacher_router.post("/results", response_model=StudentSubjectResultResponse)
async def upsert_teacher_result(payload: StudentSubjectResultUpsert, db: DbSession, current_teacher: CurrentTeacher) -> StudentSubjectResultResponse:
    await _ensure_teacher_can_write_result(db, current_teacher, payload)
...
async def list_teacher_results(db: DbSession, current_teacher: CurrentTeacher, class_id: UUID | None = Query(default=None), academic_session_id: UUID | None = Query(default=None), academic_term_id: UUID | None = Query(default=None)) -> StudentSubjectResultListResponse:
    items, total = await StudentAcademicService.list_results(db, current_teacher, class_id=class_id, academic_session_id=academic_session_id, academic_term_id=academic_term_id)
    return StudentSubjectResultListResponse(items=items, total=total)


@student_router.get("/results", response_model=StudentSubjectResultListResponse)
async def list_my_results(db: DbSession, current_student: CurrentStudent) -> StudentSubjectResultListResponse:
...
async def list_my_subject_cards(db: DbSession, current_student: CurrentStudent) -> StudentSubjectCardListResponse:
...
async def list_child_results(student_id: UUID, db: DbSession, current_parent: CurrentParent) -> StudentSubjectResultListResponse:
    items, _ = await StudentAcademicService.list_results(db, current_parent, student_id=student_id)
    return _submitted_result_list(items)
