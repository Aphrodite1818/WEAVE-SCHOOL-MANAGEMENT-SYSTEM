...
async def test_claim_only_superadmin_token_does_not_bypass_lockdown(monkeypatch):
    ...
    monkeypatch.setattr(
        platform_lockdown, "AsyncSessionLocal", lambda: SessionContext()
    )
    monkeypatch.setattr(
        platform_lockdown.SecurityResponseService, "is_ip_blocked", not_blocked
    )
    monkeypatch.setattr(
        platform_lockdown.PlatformControlService, "get_state", active_lockdown
    )
