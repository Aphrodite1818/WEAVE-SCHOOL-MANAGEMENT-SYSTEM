...
async def test_segment_dependency_counts_returns_all_evidence_counts() -> None:
    ...
    count_results = [
        SimpleNamespace(scalar_one=lambda count=count: count) for count in (2, 3, 4, 5, 6, 7)
    ]
    db = SimpleNamespace(execute=AsyncMock(side_effect=[enrollment_result, *count_results]))
