...
@pytest.mark.asyncio
async def test_admin_student_profile_update_ignores_explicit_nulls() -> None:
    ...
    with (
        ...
        response = await StudentService.update_student_profile(
            db=db,
    ...
    assert student.state_of_origin == "Lagos"
    assert response.profile_status == StudentProfileStatus.COMPLETE


@pytest.mark.asyncio
async def test_admin_student_profile_update_applies_explicit_values() -> None:
    tenant_id = uuid.uuid4()
    student = _student(tenant_id)
    db = AsyncMock()
...
@pytest.mark.asyncio
async def test_student_self_update_ignores_explicit_nulls() -> None:
    tenant_id = uuid.uuid4()
    student = _student(tenant_id)
    db = AsyncMock()
    ...
    with (
        ...
        await StudentService.update_my_student_profile(
            db=db,
            actor=student,
            payload=StudentSelfUpdate(first_name=None, gender=None),
        )
