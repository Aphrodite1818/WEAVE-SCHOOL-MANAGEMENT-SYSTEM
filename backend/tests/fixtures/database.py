...
async def _create_test_tables(test_engine: AsyncEngine) -> AsyncGenerator[None, None]:
    """Ensure the current ORM tables exist on the already migrated test database.

    The CI workflow upgrades its isolated PostgreSQL database to Alembic head before
    pytest starts. Tests deliberately do not repair obsolete columns or resurrect
    removed contracts: schema drift must fail in Alembic validation instead of being
    hidden by compatibility SQL in the fixture.
    """
